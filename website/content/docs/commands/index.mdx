...
- `-ferry-port=<uint>` - Indicates the port on which the ferry daemon should listen.
The default port is 9300.
You can also specify the port using the BOUNDARY_FERRY_LISTENING_PORT environment variable.
...
- `-skip-ferry-daemon` - If set, skips sending the auth token used for the command to the ferry daemon, if it is running.
The default value is `false`.
You can also specify this value using the BOUNDARY_SKIP_FERRY_DAEMON environment variable.
...
### `BOUNDARY_FERRY_LISTENING_PORT`

The port on which the ferry daemon should listen.
The default port is 9300.
You can use the `BOUNDARY_FERRY_LISTENING_PORT` variable or the `-ferry-port=<uint>` connection option.
...
### `BOUNDARY_SKIP_FERRY_DAEMON`

Prevents Boundary from sending the auth token used for the command to the ferry daemon, if it is running.
You can use the `BOUNDARY_SKIP_FERRY_DAEMON` variable or the `skip-ferry-daemon` connection option.
