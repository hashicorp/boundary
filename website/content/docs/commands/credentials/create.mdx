...
description: >-
  The "credentials create" command creates new credential resources. The available types are `json`, `ssh-private-key`, `username-password`, and `username-password-domain`.
---
...
#### Username password domain (Beta)

The `credentials create username-password-domain` command lets you create a username, password, and domain credential type for Active Directory authentication.

**Note:** Username-password-domain credentials are currently in beta and are primarily used with RDP targets for credential injection.

#### Example

The following example creates a username-password-domain credential in a credential store with the ID `csvlt_1234567890`:
...
-username (string: "") - The username associated with the credential. Can include domain information in username@domain or domain\username format.

-domain (string: "") - The Active Directory domain name. Optional if domain is specified within the username field.
</Tab>
</Tabs>
