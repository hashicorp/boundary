...
- `-auth-source` `(string: "")` - The authentication database for MongoDB.
If omitted, mongosh defaults `authSource` to the database specified in the connection string (`-dbname`). If no database is specified, it defaults to `admin`.
