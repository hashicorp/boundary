...
When we connect to a target a session is created in boundary. To demonstrate what a session looks like we are going to connect to an ssh based target.
...
For more information regarding different ways to connect to a target behind boundary see [Connect to Target](/docs/getting-started/connect-to-target) under Getting Started and the Advanced Session Establishment section below.

## View Sessions

View all sessions being managed by boundary in the project p_1234567890 by listing them.
...
Above we discussed connecting to a target using the boundary connect command. In addition to this we can create a session to a target and connect to that session in seperate steps. This is accomplished using the boundary target authorize that you are able to connect to a target which results in an Authorization Token being returned which can be used to connect to the session through the proxy.
