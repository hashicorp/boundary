...
Now, when you run `ssh <user>@localhost`, SSH will invoke `boundary connect`, and will tunnel the traffic through 
the local Boundary proxy in a wrapped netcat session.

## Authenticate to Boundary

Before you can invoke `boundary connect` you need to authenticate. In this example, we're using `boundary dev` with the 
