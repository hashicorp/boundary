...
In this example, we show how to create a role in Boundary, assign that role a permission set using grants, and assign principals to the role.

<Note>

All resource IDs in this guide are examples. IDs are uniquely generated for every resource upon creation, with except the resources created when testing using Boundary's `dev` mode. Please make sure to use your own resource IDs when following this example. For example, if you execute `boundary roles create`, use the resource ID of the role found in stdout, not the ID in the example command.
...
1. Navigate to an org, then to the roles view.
1. Choose the **New** button.
1. Fill the role details, including the Name `List all resources`.
1. Click **Save**. You at then directed to the role's Settings page.

![How Boundary Works](/img/ui/new-role_light.png#light-theme-only)
![How Boundary Works](/img/ui/new-role_dark.png#dark-theme-only)

</Tab>
<Tab heading="Terraform" group="terraform">

Before creating a role, ensure the principals you wish to assign to the role exist.
...
![How Boundary Works](/img/ui/add-role-scopes_light.png#light-theme-only)
![How Boundary Works](/img/ui/add-role-scopes_dark.png#dark-theme-only)
...
Grants describe the actions that the principals are allowed to perform. For more information on how to format grants, refer to [Permission Grant Formats](/boundary/docs/concepts/security/permissions/permission-grant-formats).
...
1. Paste in the example grant: `ids=*;type=*;actions=no-op,list` 
1. Click **Add**.
1. Click **Save** to commit your grant changes.

![How Boundary Works](/img/ui/add-role-grant_light.png#light-theme-only)
![How Boundary Works](/img/ui/add-role-grant_dark.png#dark-theme-only)

</Tab>
<Tab heading="Terraform" group="terraform">

Grants are assigned with the `grant_strings` attribute.
...
- `this` (this scope)
- `children` (all direct children of the assigned scopes, `global` and org scopes only)
...
Grants are automatically assigned the `this` scope when created. 

<Tabs>
<Tab heading="CLI" group="cli">

The following command adds the IDs of a project scope and the direct children of the assigned scopes to the role.
...
1. Click on the **Scopes** tab. The `this` scope will already be applied.
1. From the **Manage** dropdown, click **Manage Scopes**.
1. Click **Manage custom scopes**.
1. Select any additional scopes that should be assigned this grant, such as `p_1234567890` in Boundary's `dev` mode.

<Note>

  The settings defined for the role's children or descendants will apply to any scope assigned to this role.

</Note>

1. Click **Apply changes to org**.
1. Toggle the switch beside **Add all children**.
1. Click **Save**. The selected scope IDs and `children` will be listed under the Scopes tab.

![How Boundary Works](/img/ui/how-boundary-works_light.png#light-theme-only)
![How Boundary Works](/img/ui/how-boundary-works_dark.png#dark-theme-only)
...
The following example adds the assigned scope's id with `this`, the ID of a project scope, and the direct children of the assigned scopes to the role.
