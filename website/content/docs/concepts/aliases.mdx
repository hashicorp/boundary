...
Exampes of valid aliases include `webserver` and `webserver.boundary`.
...
When you connect to a target using the terminal, you must reference the target ID or target name and scope name. As an example, to SSH to a target, you can type `boundary connect ssh -target-id ttcp_123456789`.
...
An alias is a globally unique, DNS-like string that is associated with a destination resource.
The alias `value` parameter does not have to be delimited by a suffix, and can be just a hostname.
Exampes of valid aliases include `webserver` and `webserver.boundary`.
...
- [Create a new alias for an existing target](/boundary/docs/concepts/aliases#create-a-new-alias-for-an-existing-target)
- [Create a new target and alias at the same time](/boundary/docs/concepts/aliases#create-a-new-target-and-alias)
- [Associate an existing alias with a target](/boundary/docs/concepts/aliases#associate-an-existing-alias-with-a-target)

### Create a new alias for an existing target
