...
Single-word aliases (such as `webserver` as opposed to `webserver.boundary`) are not recommended, as they do not work intuitively on Windows.
Windows DNS resolution does not support resolving unqualified single word DNS hostnames.
Making the hostname fully qualified will work, but is unintuitive to most users. For example:
...
will not. For this reason, if any Windows users are expected to use an alias, it is recommended that it contains a dot (`.`) anywhere in the value.

See the [Transparent Sessions](/boundary/docs/concepts/transparent-sessions) topic for more information.
