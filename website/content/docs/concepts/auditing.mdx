...
Boundary provides auditing capabilities via [session recording](/boundary/docs/session-recording).
...
The worker stores the session recording on local disk during the recording phase, and then moves the recording to the external object store when the session is terminated. Session recordings are stored in the [BSR](/boundary/docs/session-recording/data/bsr-file-structure) (Boundary Session Recording) format.