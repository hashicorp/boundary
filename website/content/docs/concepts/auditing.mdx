...
`SHA256SUM` and `SHA256SUM.sig` files are to be used for cryptographically verifying the contents of this directory.
For more information on `*.sign`, `bsrKey.pub`, `wrappedBsrKey`, and `wrappedPrivKey` files, refer to the [Validating the Integrity of Session Recordings](#validating-the-integrity-of-session-recordings) paragraph.
...
Boundary Session Recording files are written to storage and are wrapped by the KMS you configured. Each session recording 
has its own individual key. Boundary generates the following keys when a session recording is authorized:

- BSR key is a plaintext AES-GCM key. (This is not uploaded to the external object store)

- Private and Public key pair is a ed25519 key pair. (The key pair is not uploaded to the external object store)
...
- `wrappedBsrKey` is the BSR key wrapped by the external KMS AES-GCM key configured by you.

- `wrappedPrivKey` is the Private ed25519 key wrapped by the external KMS AES-GCM key configured by you.
...
A BSR’s key is encrypted using the `go-kms-wrapping` package, and therefore the encrypted BlobInfo will includes the metadata required to identify the key-version used during encryption.
...
You can follow these steps to validate a session recording:

1. Unwrap `wrappedBsrKey` using the external KMS configured by you to retrieve the BSR key.
2. Unwrap `wrappedPrivKey` using the external KMS configured by you to retrieve the Private key.
3. Use the BSR key or the Private key to verify the `bsrKey.pub` key using `go-kms-wrapping` HmacSha256(...)
4. Once verified, you can use the `bsrKey.pub` key to verify the BSR SHA256SUM file using `go-kms-wrapping` ed25519.Sign(...)
...
S3 object is returned to the Boundary worker. The Boundary worker will calculate the SHA256 checksum value of the BSR file's content from
local disk and compare it to the plugin value. This process ensures that no tampering of BSR files occurs between the worker, plugin, and S3.
...
cryptographically verifies BSR directory's contents.
