...
Al sessions have a fixed time limit, and all connections are proxied through a worker.
...
The worker stores the session recording on local disk during the recording phase, and then moves the recording to the external object store when the session is terminated.
...
If no worker can access the external object store, the session is canceled and an error is returned.
...
The player retrieves data from a controller.
The controller retrieves the contents of a recording from a worker with access to the external object store.
The controller then decodes the contents of a recording into a format that is usable by the player.

## Storage buckets

A resource known as a storage bucket is used to store the recorded sessions.
...
Before you can write a file to a storage bucket, you must configure a worker with local storage to act as a cache.
The entire recording is written to the local storage cache before it is uploaded.
Once the session is complete, the files are automatically synced with the external storage bucket.

Note that a storage bucket can only belong to one scope, either the Global scope or an Org scope.
A storage bucket that is associated with the Global scope can be associated with any target.
However, a storage bucket in an Org scope can only be associated with targets from the same Org scope.
...
To enable storage recording, you must first create a storage bucket.

## BSR directory structure

The BSR (Boundary Session Recording) is a specification that defines a hierarchical directory structure of files and a binary file format.
It contains all the data transmitted between a user and a target during a single session.
The top-level directory is named with a `.bsr` suffix.

Each BSR contains a SHA256SUMS and SHA256SUMS.sign file that can be used to cryptographically verify the BSR's contents.
...
The BSR is validated based on the contents in the directory.
The top-level directory also contains BSR keys for use in decrypting and verifying the BSR contents.

The connections directory contains a summary of connections, as well as inbound and outbound requests.
