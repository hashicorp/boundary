...
HCP Boundary clusters automatically deploy HCP-managed workers which can be used as ingress workers. Using HCP-managed workers as ingress workers are useful when organizations have strict networks security policies that prohibit any inbound access. In this scenario, intermediatory or egress workers within the private network can establish a reverse proxy connection to the HCP-managed ingress worker.  
