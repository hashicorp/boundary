...
In order to configure end user traffic to ingress through HCP-managed workers, you will need to configure the self-manage worker (enterprise version). On your self-manage worker that you are using for egress to HCP-manage worker, set the coniguration file with the following parameters:
...
To route traffic through the HCP-managed workers, you should set the egress filters of each target to match the tag set in the self-manage worker's configuration file. There is no need to set additional ingress filters on the targets.
