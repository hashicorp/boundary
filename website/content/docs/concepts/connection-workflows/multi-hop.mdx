...
If there was a target behind Worker 3, and end users connected to Worker 1, then Worker 1 is the ingress worker and worker 3 is the egress worker.
...
When you deploy multi-hop workers, it is important to configure [worker tags](/docs/concepts/filtering/worker-tags#workers) that establish ingress and egress workers.
...
You must configure [target worker filters](/docs/concepts/filtering/worker-tags#target-workers) for workers to allow for fine-grained control over which workers handle ingress and egress for session traffic to a target.
