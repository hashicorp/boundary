...
<Note>
  ...
  Windows users will need to install the [Nmap utility](https://nmap.org/download.html) instead of netcat.
...
<CodeTabs tabs={[ "MacOS/Linux", "Windows" ]}>
...
```bash
Host ttcp_*
  ProxyCommand sh -c "boundary connect -target-id %n -exec nmap -- {{boundary.ip}} {{boundary.port}}"
```
...
Host localhost
    # requires BOUNDARY_CONNECT_AUTHZ_TOKEN to be set from authorize-session command
    ProxyCommand sh -c "boundary connect -exec nmap -- {{boundary.ip}} {{boundary.port}}"
```
