...
## Status and Health
Boundary workers report their status to controllers on a regular basis. The last time a worker reported status can be seen in the
admin UI as "Last Seen" and in the CLI as the "Last Status Time". If a worker hasn't reported status recently, this can be an
indication of an unhealthy worker.

To determine the health of a worker, the worker health endpoint can be queried. This endpoint will return the worker state, active
session count, and connection state of the worker. For more information, refer to the [health endpoints documentation](/boundary/docs/operations/health).
