...
Workers can [authenticate](/boundary/docs/secure/encryption/connections-tls#pki-based-worker-authentication) directly to the control plane or through an upstream worker to the control plane. Authenticating through an upstream worker is also referred to as "multi-hop worker authentication."
...
recording](/boundary/docs/session-recording/). For session recording, workers also write the recorded session contents directly to the [storage
bucket](/boundary/docs/session-recording#storage-buckets).
...
can [filter](/boundary/docs/secure/worker-tags) through, to find the appropriate worker to use for a session. For example, Boundary could filter to workers
...
session count, and connection state of the worker. For more information, refer to the [health endpoints](/boundary/docs/monitor/health) documentation.
...
Refer to the [multi-hop sessions](/boundary/docs/targets/configuration/multi-hop) documentation for more information.
