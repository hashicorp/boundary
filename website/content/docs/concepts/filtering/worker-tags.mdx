...
Tags can be used to control which [PKI workers] can handle Vault requests by specifying
a `worker_filter`attribute when configuring [credential stores].

This allows for the use of private Vault instances with Boundary. PKI workers
