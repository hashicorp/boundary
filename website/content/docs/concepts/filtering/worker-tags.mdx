...
the worker is not included in the final set. Be careful when using negative
matching (such as `!=`). For instance, if the key is not found when evaluating
`"/foo" != "bar"`, the expression will return false, which will be the same
behavior as if the key was found but equal.
