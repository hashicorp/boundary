...
Example of adding a filter to a target with the CLI:
```
...
Example of adding a filter to a Vault credential store with the CLI:
```
...
Example of a worker filter configured on a storage bucket in Admin UI:
...
Example of adding a filter to a storage bucket with the CLI:
```
