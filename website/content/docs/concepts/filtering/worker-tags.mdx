...
Worker tags can be configured using the worker config file, the Admin UI, the CLI, or the API. Select a tag below to view examples.

<Tabs>
<Tab heading="Worker config file" group="file">

Workers can be tagged with a set of key/value tags in their configuration file. The keys and values can be any lower-cased printable value. Each key can have more than one value:
...
- For setting the entire `tags` block, both the keys and values must be specified in JSON or HCL format:
...
Workers can be tagged with a set of key/value API tags in the Boundary Admin UI. The keys and values can be any lower-cased printable value. Each key can have more than one value:
...
In the image above, the tag type is `config`, which means the tags were defined in the worker config file. Config tags cannot be modified within the UI, and must be updated in the config file itself.
...
API tags can be added in the Admin UI. API tags are defined on the Boundary controller, and are not updated in the worker's config file.
...
![Worker API in Boundary Admin UI](/img/ui/worker-tags-api_light.png#light-theme-onlyapi
![Worker API in Boundary Admin UI](/img/ui/worker-tags-api_dark.png#dark-theme-only)

API tags can be removed using the Admin UI, CLI, or API.

</Tab>
<Tab heading="CLI" group="cli">

Workers can be tagged with a set of key/value API tags using the CLI. The keys and values can be any lower-cased printable value. Each key can have more than one value:
...
In the output above, the tags listed as `Configuration`, were defined in the worker config file. Config tags cannot be modified using the CLI, and must be updated in the config file itself.
...
API tags can be added using the Admin UI, CLI, or API. API tags are defined on the Boundary controller, and are not updated in the worker's config file.

API tags can be added using the [boundary workers add-worker-tags](/boundary/docs/commands/workers/add-worker-tags) command using the CLI:
...
API tags can be removed using the Admin UI, CLI, or API. To remove an API tag using [boundary workers remove-worker-tags](/boundary/docs/commands/workers/remove-worker-tags)
