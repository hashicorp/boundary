...
Create a service account in GCP and download the private key file. The service account must have the following roles:
...
This is useful when you want to restrict the permissions of the service account used by Boundary. Two service accounts are required:

- `Base service account`: The service account used by Boundary to authenticate with GCP. The service account must have the following roles:
  - `roles/iam.serviceAccountTokenCreator` (Required) - To create a token for the target service account.
  - `roles/iam.serviceAccountKeyAdmin` (Optional) - To rotate the service account key if `disable_credential_rotation` is set to `false`.

- `Target service account`: The service account to impersonate. The service account must have the following roles:
  - `roles/compute.viewer` (Required): To list Compute Engine VM instance in the project.
...
If you are running Boundary on a GCP VM instance, you can use Application Default Credentials (ADC) to authenticate with GCP.
Boundary will use the service account associated with the VM instance to authenticate with GCP.
...
- `zone` (Required): The deployment area within a region. All host sets in this
  catalog will be configured for this zone.
- `project_id` (Required): The project ID associated with the service account.
- `disable_credential_rotation` (Optional): When set to `true`, Boundary will not rotate the credentials with GCP automatically.
- `client_email` (Optional): The email address associated with the service account. 
  The email address used to uniquely identify the service account.
- `target_service_account_id` (Optional): The email address of the service account to impersonate.  
...
The `filters` field contains string filters in the format key=val1. The key corresponds to
  ...
  When multiple filters are provided, the plugin will apply a logical `AND` operation to the filters.

  If there is no explicit instance status filter, the default status will be set to `status="RUNNING"`. 
  This ensures that we filter on running instances only, saving time when processing results.

  Labels are recommended to be used as filters as they are more flexible and can be used to filter on multiple values
