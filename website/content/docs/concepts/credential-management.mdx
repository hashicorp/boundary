...
You can attach brokered credentials to SSH, TCP, or RDP (BETA) targets.
Brokered credentials can take the form of a token, username password, username password domain, SSH private key, certificate, JSON blob, or an unstructured secret stored in Vault, for example.
...
You can inject username password domain credentials when you access RDP targets in Windows environments that use Active Directory for authentication.
