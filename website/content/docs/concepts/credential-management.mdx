...
To configure credential brokering or credential injection with static credentials, refer to [Create a static credential store](/boundary/docs/credentials/static-cred-boundary) and [Create a Vault credential store](/boundary/docs/credentials/static-cred-vault).
