...
- `insecure_tls` - (optional) If true, skips LDAP server SSL certificate
  validation - insecure and use with caution. Defaults to false.

- `discover_dn` - (optional) If true, use anon bind to discover the bind DN of a
...
- `urls` - (required) The LDAP URLS that specify LDAP servers to connection to.
...
- `user_dn` - (optional) If set is the base DN under which to perform user
  search. Example: ou=Users,dc=example,dc=com

- `user_attr` - (optional) If set is the attribute on user attribute entry
  matching the login-name passed when authenticating.  Examples: cn, uid

- `user_filter` - (optional) If set is a go template used to construct a LDAP
...
- `group_dn` - (optional) If set is the base DN under which to perform a group
  search. Example: ou=Groups,dc=example,dc=com

  Note: there is no default, so no base dn will be used for group searches if
  it's not specified

- `group_attr` - (optional) If set is the LDAP attribute to follow on objects
...
- `group_filter` - (optional) If set is a Go template used when constructing the
...
- `certificates` - (optional) If set are PEM encoded x509 certificates in ASN.1
  DER form that can be used as trust anchors when connecting to an LDAP
  provider. 

- `client)certificate` - (optional) If set is a PEM encoded x509 certificates in
  ASN.1 DER form to be used as a client certificate. It must be set if the
  optional client_certificate_key is specified

- `client_certificate_key` - (optional) If set is a certificate key in PKCS #8,
  ASN.1 DER form. It must be set if the optional client_certificate is
  specified.
  
- `bind_dn` - (optional) If set is the distinguished name of entry to bind when 
  performing user and group searches. Example:
  cn=vault,ou=Users,dc=example,dc=com 

- `bind_password` - (optional) If set is the password to use along with bind_dn
...
- `account_attribute_maps` - (optional) If set are attribute maps from custom
