...
- `full_name` - (output only)
  ...
  time the user successfully authenticates. It will be empty until the user's
  first successful authentication.

- `email` - (output only)
  Maps to the email address attribute for the authenticated user and is updated
  every time the user successfully authenticates. It will be empty until the
  user's first successful authentication.

- `dn` - (output only)
  The  distinguished name for the authenticated user and is updated
  every time the user successfully authenticates. It will be empty until the
  user's first successful authentication.
  
- `member_of_groups` - (output only)
  A list of the group's the authenticated user is a member of. It will be empty
