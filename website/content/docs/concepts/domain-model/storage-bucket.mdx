...
A storage bucket can only belong to one [scope][], either the Global scope or an Org scope.
A storage bucket that is associated with the Global scope can be associated with any target.
Howver, a storage bucket in an Org scope can only be associated with targets in a project from the same Org scope.
...
- `worker_filter` - (Required) A filter to the worker or workers that can process requests for the storage bucket.
