...
benefits for new Boundary users and users who don't require Boundary's
flexibility and scalability, as it's less complex to set up and understand.
Because of its simplicity, using this path on environments where a significant
number of resources exist is not recommended.
...
address on a target with Boundary's flexibility and scalability for discovering
and categorizing networked resources. We recommend this path for setting
up Boundary in environments where a significant number of resources exist.
