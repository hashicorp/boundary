...
- `egress_worker_filter` - (optional)
  ...
  You can configure an egress filter to enable [multi-hop](/docs/configuration/worker/pki-worker#multi-hop-workershcp-only) connections.
...
- `ingress_worker_filter` - (optional) <sup>HCP Only</sup>
  ...
  If you do not configure an ingress filter, Boundary selects an HCP worker for the session.
