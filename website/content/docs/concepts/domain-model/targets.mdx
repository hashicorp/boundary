...
- **To use brokered credentials to connect to an SSH target**: you must use a `tcp` target type.
- **To use injected application credentials to connect to an SSH target**: you must use an `ssh` target type.
- **To enable session recording for an SSH target**: you must use injected application credentials and an `ssh` target type.
...
[credentials][]. If you use injected credentials, Boundary injects credentials into the SSH session between a client and end host. Injected credentials allow users to
securely connect to remote hosts using SSH, while never being in possession of a valid credential for that target host.

<Note>

If you set the target `type` to `ssh`, you must use brokered or injected credentials.
You cannot reference a static .pem file.
