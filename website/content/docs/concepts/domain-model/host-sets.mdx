...
A host set is a resource that represents a collection of [hosts][]
which are considered equivalent for the purposes of access control.
A host set can only include hosts from the same [host catalog][] as
the host set. A [target][] can reference host sets from host catalogs
...
- `preferred_endpoints` - (optional)
  ...
  or `dns:<globbed name>` used to select the addresses of a [hosts][] when
...
- `sync_interval_seconds` - (optional)
  The number of seconds between the time boundary syncs the [hosts][] in this set
  using this host set's plugin.
