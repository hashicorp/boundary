...
A Static credential store is managed by Boundary and allows for a user-supplied credential
and stores it (encrypted) directly in Boundary. Currently, the static credential store can
hold credentials of type username_password.
