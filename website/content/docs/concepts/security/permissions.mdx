...
publicly disclosing more information than users might be desire.
...
However, since grants compose, if both of the above grants are included, the
final set of output fields will be `id,scope_id,name,description` for `list`
(and `no-op`) actions, and `id` for all other actions.

If, after the grants are composed for a given request, none of the grants
contain `output_fields` definitions (or if they do not apply to the
resource/action in question), the defaults are used. These defaults are
implemented using internal values for `output_fields` and vary based on whether
the user is the anonymous user:
...
~> `output_fields` is ommitted in most example below for brevity but are valid
