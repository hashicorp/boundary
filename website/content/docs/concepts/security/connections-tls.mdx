...
description: How Boundary secures its connections
---
...
1. The Worker marshals the TLS chain and nonce and encrypts the resulting bytes
   via the shared KMS. This value is marshaled and split into chunks.

1. The Worker establishes a TLS 1.3 connection to the Controller. The encrypted
   value is transmitted to the Controller via the TLS ALPN field as numbered
   chunks.

1. The Controller reads the chunks and reassembles them into the original
   encrypted value.

1. The Controller decrypts this value via the shared KMS. If successful, the
...
1. The Controller uses the decrypted parameters to configures its TLS stack with
   the same certificate and key.

1. The connection is mutually authenticated; on each end, only the single
...
1. The certificate and private key (along with other session authorization data,
...
1. The client (that is, the `boundary connect` command) parses this session
...
1. The worker sees the SNI value and makes a call to the Controller to fetch
   session authorization information, keyed by the session ID.

1. The Controller looks for a session with the given ID and fetches the
...
1. The Worker uses the given data to construct a TLS stack with the same
   certificate and key.
...
1. If successful, the client and Worker perform a handshake, where the client
