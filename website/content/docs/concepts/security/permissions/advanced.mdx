...
the final set of output fields will be `ids,scope_id,name,description` for `list`
(and `no-op`) actions, and `ids` for all other actions.
...
visibility). If further use cases arise from user feedback, we can expand the list of permitted actions.
