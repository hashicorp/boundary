...
With the [OIDC](/boundary/tutorials/identity-management/oidc-auth) and LDAP auth methods, Boundary automatically generates accounts and users in the same scope
as the auth method, using data provided by the identity provider. The accounts and users are only created once the user authenticates to Boundary for the first time. 
