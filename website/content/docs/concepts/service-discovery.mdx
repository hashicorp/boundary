...
Boundary supports target/service discovery in three primary workflows: <br>
...
Boundary dynamic host catalogs create a pluggable workflow to automate the ingestion of infrastructure 
resources at Boundary hosts from cloud providers service registries, and other 
infrastructure providers. This removes the need to know host connection info or 
reapply infrastructure as code templates to configure new or changed resources.

# Dynamic Host Catalogs
Dynamic host catalogs are an agentless, pluggable workflow for Boundary to 
