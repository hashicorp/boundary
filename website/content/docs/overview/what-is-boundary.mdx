...
HashiCorp Boundary is an agentless identity-aware proxy. Boundary is intended to simplify and secure enabling least-privileged access to cloud infrastructure.
...
- Record and manage privilege sessions
...
- **HCP Boundary**: a managed offering of Boundary with commercial-only use-cases features. Boundary's control plane is hosted by HashiCorp and customers have the option of running private workers within their environment.
- **Boundary OSS**: an open source, self-managed version of Boundary.
- **Boundary Enterprise**: a self-managed offering of Boundary with full feature parity to HCP Boundary.
...
Regardless of which server edition of Boundary you are hosting, all editions use the same Desktop and CLI clients, which you can download [here](https://developer.hashicorp.com/boundary/downloads).
