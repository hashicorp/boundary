...
It also gives a brief walkthrough of the end user's experience.
  ...
  <video controls>
    <source src="https://www.datocms-assets.com/2885/1686339121-how-boundary-works-v2.mp4" type="video/mp4" />
  </video>
...
Boundary provides [auditing](/boundary/docs/concepts/auditing) features via [session recording](/boundary/docs/configuration/session-recording) to help organizations address compliance and threat management needs.

When [session recording](/boundary/docs/configuration/session-recording) is enabled on a target, any sessions established to that target are recorded.
Recorded sessions are stored in an admin-specified storage backend.
Administrators can access recorded sessions at a later time using a session player that runs in a browser.
...
Boundary offers three types of deployment options.
The first two options are self-managed deployments.
A self-managed approach enables organizations to proxy all session data through their own networks, while still providing the convenience of a managed service.
HashiCorp offers Boundary in both open source (OSS) and enterprise self-managed versions.
