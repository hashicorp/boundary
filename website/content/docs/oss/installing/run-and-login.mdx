...
`boundary dev`. The auth method resource resides in the global scope and is set
as the primary for that scope, which means you don't need to specify the
auth method ID as a flag when using the authenticate command, all you need
is the short form `boundary authenticate`.
