...
__Note__: `name` and `description` fields are not valid config fields for Self-Managed workers. These fields are only valid for
[KMS Workers][]. `name` and `description` can only be set for Self-Managed workers through the API.
