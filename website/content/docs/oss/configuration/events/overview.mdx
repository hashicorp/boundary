...
- `sink` - Specifies the configuration of an event sink. Currently, two types of
  sink are supported: [file](/docs/oss/configuration/events/file) and [stderr](/docs/oss/configuration/events/stderr). If no sinks are configured then all
  events will be sent to a default [stderr](/docs/oss/configuration/events/stderr) sink. Events may be sent to multiple
  sinks.
