...
For example, Boundary workers version 0.17.0 are compatible with Boundary control plane running Boundary 0.18.0.
...
For example, Boundary clients version 0.17.0 are compatible with Boundary control plane running Boundary 0.18.0.
However, they will not have compatibility once the control plane is updated to version 0.19.0 or above.
Boundary clients version 0.19.0 are not compatible with Boundary control plane running Boundary 0.18.0 or lower.
Customers are recommended to run the latest versions of Boundary in order to leverage the newest features and bug fixes.

Prior to Boundary version 0.19.3, the Desktop client used a different numbering scheme than the CLI and control plane.
Refer to the table for the Desktop version that corresponded to the control plane version.
...
For example, the Desktop version 2.0.3 was compatible with version 0.17.0 of the control plane.
But when the control plane was upgraded to 0.18.0, version 2.0.3 was no longer officially supported.
