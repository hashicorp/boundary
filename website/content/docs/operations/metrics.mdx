...
"ops" purpose. By default a listener with that purpose runs on port 9203.  See
the [sample code](#sample-code) section for an example listener stanza in a
`config.hcl` file.

Metrics are important for understanding what is happening in a boundary
deployment, enabling diagnoses of deployed controllers and workers. Adding
metrics will grant greater visibility into the various components of a running
Boundary deployment through Prometheus.

For more information about Prometheus, see the [Prometheus Overview
Guide](https://prometheus.io/docs/introduction/overview). The Prometheus Go
library is used to define and export metrics from the Boundary system. The
metrics uses the [OpenMetric
...
The following lists the current names and descriptions of metrics provided for controllers.

| Name                                                          | Description                                   |
|---------------------------------------------------------------|-----------------------------------------------|
| `boundary_controller_api_http_request_duration_seconds`       | Historgram of latencies for HTTP requests.     |  
...
| `boundary_controller_cluster_grpc_request_size_bytes`         | Histogram of byte sizes for requests made to the gRPC service running on the cluster listener. |
| `boundary_controller_cluster_grpc_response_size_bytes`        | Histogram of byte sizes for responses from the gRPC service running on the cluster listener. |


### Worker

The following lists the current names and descriptions of metrics provided for
workers.
...
Refer to _TBD_ for a step-by-step tutorial.
