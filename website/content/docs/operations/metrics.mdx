...
The following lists the current metrics exported, their descriptions, and full names.
...
| Metric        | Description                                    | Metric name   |
|---------------|------------------------------------------------|---------------|
| Time Until Header Written | Histogram of time elapsed after the TLS connection is established to when the first http header is written back from the server | `boundary_worker_proxy_http_write_header_duration_seconds` |
