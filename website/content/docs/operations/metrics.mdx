...
Boundary provides Prometheus metrics for Controllers and Workers at the ops endpoint on **port 9203** through the `/metrics `path.  For more information about Prometheus, see the **Prometheus Overview Guide** at https://prometheus.io/docs/introduction/overview. The Prometheus go library will be used to define and export metrics from the Boundary system. The metrics uses the [OpenMetric exposition](https://prometheus.io/docs/instrumenting/exposition_formats/#openmetrics-text-format) format.
