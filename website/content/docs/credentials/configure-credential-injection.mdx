...
- **RDP targets**: Protocol-aware RDP connections (Enterprise, Beta)

**Note:** RDP credential injection is currently in beta.
...
- You must have an existing target available that supports credential injection:
  ...
  - **RDP targets** (Beta): Must be configured with at least one injected application credential
...
- **For RDP targets** (Beta): Network Level Authentication (NLA) is supported. Kerberos and NTLMv2 authentication methods are supported for domain-joined servers.
