...
<table>
  ...
  <tr>
    <td style={{verticalAlign: 'middle'}}>
    <code>curl</code> no longer included in Boundary Enterprise
    </td>
    <td style={{verticalAlign: 'middle'}}>
    As of version 0.16.3 and later, the <code>curl</code> binary is no longer included in the published Docker container image for Boundary Enterprise. If your workflow depends on having <code>curl</code> in the image, you can dynamically install it using <code>apk</code>.
    <br /><br />
