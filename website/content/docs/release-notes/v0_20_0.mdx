...
<table>
  ...
  <tr>
    ...
    <td style={{verticalAlign: 'middle'}}>
    Boundary now supports RDP targets. RDP targets let you use injected credentials to authenticate an RDP session between the client and end host in Windows environments that use Active Directory authentication. RDP targets use the new username password domain credential type.
    <br /><br />
