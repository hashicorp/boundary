...
<Note>
  ...
  Self-managed versions of Boundary v0.13.0 will be available to customers before HCP Boundary is.
  HCP Boundary v0.13.0 specific features will be available shortly.
...
- HCP Boundary maintenance window (HCP only)
- OIDC authentication improvements

## New features

**Boundary Enterprise**: Boundary OSS allows users to deploy and test out Boundary in their own environments, but it does not include enterprise capabilities.
HCP Boundary does include enterprise features and is offered as a cloud-managed solution, making it simple to deploy and maintain.
But many highly regulated organizations are prohibited from adopting managed solutions.

In this release, we introduce Boundary Enterprise.
Boundary Enterprise has the same features as HCP Boundary, but you can deploy it in your own public or private cloud.
Further, Boundary Enterprise comes with seamless migration support from OSS.
...
**LDAP authentication method <sup>Preview</sup>**: This release introduces a new Lightweight Directory Access Protocol (LDAP) authentication method as a preview feature.
...
**OIDC authentication improvements**: A new flag was added to the OIDC authentication command, `output-auth-url`.
If you provide the flag, you can copy and paste the authentication URL that is printed out into any browser to complete the authentication flow.
The resulting token is then passed to the CLI.

**Shared KMS workers**: KMS workers now have feature parity with PKI workers.
You can use more than one KMS for authenticating downstreams across different networks.
...
**Dynamic host catalog external name**: You can now view the name of the external host on the **Host Catalogs** page.

## Deprecations and changes

Boundary version 0.13.0 has the following deprecations and changes:

- Due to the update to KMS workers, the previous KMS auth method is deprecated.
