...
- `503`: Boundary returns a `503` if it is unable to store a quota due to the API rate limit being exceeded.
...
You can configure the amount of memory that is reserved for tracking quotas using the `api_rate_limit_max_quotas` variable.
There are also two metrics that allow you to monitor quota tracking:

- `boundary_controller_api_ratelimiter_quota_stroage_capacity`
...
By default, the `list` action has a limit of 120 requests per minute for all resources.
All other actions have a default limit of 3,000 requests per minute for all resources.
...
controller {
  ...
  # sets an overall cap on list since it is relatively expensive.
  api_rate_limit {
	resources = ["*"]
	actions   = ["list"]
	per 	= "total"
  ...
  # Limit of all authed requests, this essentially sets a default for
  # all authed requests to prevent one user consuming all of the total limit
  # set above. Any limits that follow can override the default for
  # specific sets of resources/actions
  api_rate_limit {
	resources = ["*"]
	actions   = ["*"]
	per 	= "auth-token"
  ...
  # Limit of all list operations by auth token to set a sane default since
  ...
  # that clients will want to list frequently, but might be more expensive if
  # there is a large number of them for each user. Having a lower limit
