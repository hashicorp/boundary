...
The following is a guide for troubleshooting the behavior of the Client Agent. It is recommended to try the steps from top to bottom.

### Check the status of the Client Agent

If you are experiencing unexpected behavior, the first thing to do is to check on the status of the Client Agent.
We can do this via the Boundary CLI or the Desktop Client. To see the cliet agent status through the Boundary CLI, use the following command:
...
On the Desktop Client, the status of the Client Agent can be found by navigating to "Settings" and then scrolling down to the "Boundary Client Agent" section.
...
If the status is "paused", the Client Agent is not currently intercepting DNS requests and will need to be resumed.
The Client Agent can be paused by the user, and it will also pause itself if it detects a large number of network failures in a short period of time.

The status also allows you to see whether your current user is authenticated.
If the response looks something like the above, including showing an auth token ID and expiration, your current user is authenticated.
If not, you may need to first authenticate to the Client Agent using `boundary authenticate`.

The status will also sometimes contain a list of errors that have been encountered by the client-agent.
...
If attempting to see the status results in a complete error, the Client Agent may not be running.
...
The Client Agent can be resumed either via the Boundary CLI or the Desktop Client. On the CLI, run:
...
On the Desktop Client, you can resume the Client Agent through the "Resume" button on the Boundary Client Agent section of the settings.
Once the Client Agent has resumed, test if it has started working as expected again.

### Inspect the log file

If you are not able to diagnose the problem by looking at the status or resuming it, another step can be to read through the log file produced by the Client Agent for any indication of what the problem could be.
...
The list below provides some common errors and explanation for why they may be happening.

It may be necessary to increase the logging verbosity of the client-agent.
This can be accomplished by setting the `log_level` option in the configuration file to `DEBUG`.
See the section on updating the configuration for more information.
...
If the DNS requests on your system are not being sent to the right place, or they are not being answered appropriately, Transparent Sessions will not work.

We will use the `nslookup` command to understand where the DNS requests are being sent.
We can start by sending a DNS request for `hashicorp.com`:
...
Next we can try to make a DNS request to an alias that we expect to be working. Assuming we have an alias with a value of `mytarget.boundary.dev`:
...
We can tell two things from this:
1. Again, the Client Agent is likely able to intercept the DNS request, because the server is a local IPv6 address in the [ULA](https://en.wikipedia.org/wiki/Unique_local_address) range.
...
If you are not seeing this kind of response, it may be that the alias you are trying to connect to doesn't exist, or your user is not authorized to connect to it.
Double check that you are using the correct alias and that your user is authorized to connect to it.

### Flush OS DNS cache

If you are still not seeing the expected behavior, it can be useful to flush the DNS cache of the operating system.
The exact steps to use depend on the operating system you are using.
...
After flushing the DNS cache, try connecting to the alias again, or repeat the steps above.
...
- The Client Agent is not able to intercept DNS requests (if it is shut down or paused)
...
Diagnosing what is causing the error differs per operating system:
...
This usually implies that there is a problem reaching the internet or the Boundary controller used.
...
It is recommended to pause the client-agent and examine the status and logs for further indication of errors:
