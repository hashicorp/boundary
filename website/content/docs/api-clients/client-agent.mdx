...
You may notice unexpected behavior affecting grants for HCP Boundary clusters that were created before May 3, 2025.
Behavior may include issues such as authenticated users being unable to list aliases for transparent sessions.
This issue has been resolved for any clusters created after May 3.
Refer to the following section for more information and workaround options.
...
<Tabs>
<Tab heading="Default admin role" group="admin">

   Update the grant scopes on a role scopes to include the flag "Add all descendants".

   Update the Default admin role's [grant scope](/boundary/docs/common-workflows/manage-roles#add-grant-scopes) ID to apply to all `descendants`.

</Tab>
<Tab heading="Default login role" group="login">

   1. Update the scopes to include the flag "Add all descendants"
   1. Add the following grant:

      ````
      ids=*;type=auth-token;actions=read:self,delete:self
      ````

   1. Remove the following grant:
...
<Tab heading="Authenticated users" group="users">

   If it doesn't already exist, create a role applied to the `u_auth` principal on the global scope and descendants with the following grants:
