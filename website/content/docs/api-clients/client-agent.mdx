...
1. Make the changes you want to the configuration.
...
Some configuration values can be changed without restarting the Client Agent.
...
To reload the configuration, update the configuration file as described above and use the following command:

```shell-session
sudo pkill -1 boundary-client-agent
```

This sends a SIGHUP signal to the Client Agent, which causes it to reload the configuration file.
...
Note that restarting the Client Agent will close any existing sessions.

## Controlling the Client Agent
