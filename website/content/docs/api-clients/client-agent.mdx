...
Note that this will not include sessions that are not managed by the Client Agent. Use `boundary sessions list -recursive` to see all sessions.
...
We can do this via the Boundary CLI or the Desktop Client. To see the Client Agent status through the Boundary CLI, use the following command:
...
The status will also sometimes contain a list of errors (ordered by most recent first) that have been encountered by the Client Agent.
...
The list below provides some common errors and explanations for why they may be happening.
