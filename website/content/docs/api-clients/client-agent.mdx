...
## Specify a prefix for IP addresses

You can specify an alternate prefix for the Client Agent to use when it generates IP addresses.
The value must be between `/8` and `/16`.

Complete the following steps to specify a prefix for IP addresses:
...
1. Change the `v4_prefix` setting to the new prefix you want to use when you generate IP addresses.
...
## Specify a DNS request timeout

You can specify for how long the Client Agent DNS request handling, including any recursion, is allowed to run before it is canceled.

Complete the following steps to specify a DNS request timeout period:
...
1. Change the `dns_request_timeout` setting to specify the number of seconds you want to allow request handling to run for before it is canceled.
1. Save the file and reload the configuration by sending a SIGHUP signal to the process running the Client Agent with the following command:
...
You can specify which DNS servers the Client Agent should use for recursing non-Boundary requests.
The DNS servers you specify in the configuration file override any DNS servers that are configured on the system.

Complete the following steps to specify the DNS servers to use:
...
1. Change the `override_upstream_dns_servers` setting to include the DNS servers you want to use for recursing non-Boundary requests.
1. Save the file and reload the configuration by sending a SIGHUP signal to the process running the Client Agent with the following command:
...
You can specify where to write the Boundary Client Agent log file.
By default for MacOS, the log file is located in:

`Library/Application Support/HashiCorp/Boundary/boundary-client-agent.log`

For Windows, the log file is located in:

`C:\Windows\Logs\boundary-client-agent.log`

Complete the following steps to change the location in which the log file is stored:
...
1. Change the `log_file` setting to the new directory where you want to store the log file.
1. Save the file and reload the configuration by sending a SIGHUP signal to the process running the Client Agent with the following command:
...
1. Change the `log_level` setting to the new verbosity setting.
1. Save the file and reload the configuration by sending a SIGHUP signal to the process running the Client Agent with the following command:
...
By default, it is located in the following directory:
   ...
   Create a support ticket, and attach the `boundary-client-agent.log` file.
