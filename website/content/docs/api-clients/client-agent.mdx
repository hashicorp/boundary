...
When the Boundary Client Agent runs alongside an authenticated Boundary client, the agent establishes itself as the primary resolver on the client and intercepts DNS requests.
...
If you authenticate to a different Boundary controller, any existing sessions are terminated and a new transparent session would be established with the new controller.
...
- have permission to establish a session to a target or multiple targets.
...
The Boundary Client Agent configuraton file is located in the following directories:
...
- `v4_prefix` - Specifies an alternate prefix to use for generating IPs. Currently must be between /8 and /16
   ...
   v4_prefix=1.1.1.1/8
   ```
- `recurse_only` - Recurses the DNS request only, does not proxy sessions.
...
- `dns_request_timeout` - Specifies the amount of time the client will wait for a response to a DNS request.
...
Refer to the following sections to troubleshoot common issues.

### DNS does not resolve

Refer to the following steps for your operating system:
...
### Log file errors read "failed to listen for DNS on either IPv4 or IPv6"
   ...
   Refer to the following steps for your operating system to diagnose the issue:
