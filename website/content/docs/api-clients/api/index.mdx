...
Boundary's API features rate limiting to help prevent system resources from being overwhlemed by too many simultaneous requests.
For more information, refer to [API rate limiting](/boundary/docs/api-clients/api/rate-limiting).

The search and filter feature introduced pagination for the results of API searches.
Refer to [API pagination](/boundary/docs/api-clients/api/pagination) for more information about pagination and the local cache used for searches.