...
To support searching and filtering large lists of results in Boundary without overwhelming your system's resources, the Boundary Desktop client uses an API pagination feature.
...
When you reach the end of the initial list of results, you can request the next page of results using a token.
...
To request the next page of results, provide the `list_token` as a parameter in the subsequent `list` operation.
Boundary returns the next page of results with new `response_type` and `list_token` values.
If the `response_type` is `delta`, you can use the corresponding `list_token` to view the additional items.
You can continue viewing new pages of results until the `response_type` reads `complete`.

## Refresh

When the `response_type` reads `complete`, you have paged through all of the results that were available when you ran the initial `list` operation.
...
Similarly to the intial pagination, Boundary returns a `response_type` as part of the response.
If the `response_type` is `delta`, it means there are more items available in the list.
If the `response_type` is `complete`, it means the list is complete.
...
You can refresh the local results of your initial `list` operation for up to 30 days.
Maintaining a local cache of results for a frequently used `list` operation can help to reduce server cost.
After 30 days, the `list_token` expires and you must make a new, full `list` request to regenerate the results.
Boundary treats requests with tokens that have a `create_time` older than 30 days in the past as a request with an invalid token and returns an error.
...
If you make frequent updates, or if you are updating data while you are running a `list` operation, HashiCorp recommends that you instead perform a full `list` operation.
