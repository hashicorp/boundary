...
To request the next page of results, run the `list` operation again and include the `list_token` as a parameter.
...
Boundary returns a `response_type` as part of the response, similarly to when you ran the initial operation.
If the `response_type` is `delta`, it means there are more items available in the list.
If the `response_type` is `complete`, it means the list is complete.
