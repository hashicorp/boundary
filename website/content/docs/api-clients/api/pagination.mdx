...
The results of the `list` operation are cached locally to help protect your system resources.
Maintaining a local cache of results for a frequently used `list` operation can also help to reduce server cost.
You can refresh the local results of your initial `list` operation as many times as you want for up to 30 days.

## Initial pagination

When you run a `list` operation on a resource, the initial results are sorted in descending order by creation time and public ID, by default.
...
To request the next page of results, run the `list` operation again and include the `list_token` that Boundary provided as a parameter.
...
You can now refresh the cached list of results to return any resources that were updated since you started viewing the list responses.
...
The `deleted_ids` response contains the IDs of any resources that were deleted since the initial `list` operation.
...
List tokens contain general metadata associated with the ongoing pagination and either a pagination or refresh token, depending on the phase of the pagination.
If you compose an invalid token, you receive an invalid token error.

When you make a `list` request without a `list_token`, Boundary builds a page of items that contains all the resources currently available, subject to the page size, with the following attributes:
