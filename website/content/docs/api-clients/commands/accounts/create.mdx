...
- `-auth-method-id` `(string: "")` - Indicates the auth-method resource to use for the
   operation. You can also specify the auth-method using the `BOUNDARY_AUTH_METHOD_ID`
   environment variable.
- `-description` `(string: "")` - Indicates the description to set for the account.
- `-name` `(string: "")` - Indicates the name to set on the account.
...
- `-login-name` `(string: "")` - Specifies the login name for the account.
...
- `-issuer` `(string: "")` - Indicates the issuer for the account.
- `-subject` `(string: "")` - Indicates the subject for this account on the OIDC provider.
...
- `-login-name` `(string: "")` - Indicates the login name for the account
- `-password` `(string: "")` - Indicates the password for the account. If blank, the
   command prompts you to enter the password interactively in a
   non-echoing way. Otherwise, thi value can refer to a file on disk (`file://`) or an environment variable (`env://`) from which the password is read.
