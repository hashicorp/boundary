...
- `-id` `(string: "")` - Specifies the ID of the account on which to set a password.
- `-password` `(string: "")` - Indicates the password for the account. Use this value with `env://` or `file://` syntax. If you do not specify a password, the command prompts you to enter the password in a non-echoing way.
- `-version` `(int: 0)` - Indicates the version of the account against which to perform an
