...
- `-include-terminated` - Specifies whether you want terminated sessions to be included in the `list` results.
The default value is `false`.
- `recursive` - Runs the list operation recursively on any child scopes, if the type supports it.
The default value is `false`.
- `scope-id=<string>` - Indicates the scope from which to list sessions.
