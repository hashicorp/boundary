...
page details the common patterns used to help users make better use
of the CLI.
...
The Unix password manager [pass](https://www.passwordstore.org/) is also available on all platforms.
...
on. For instance, when operating on roles, the command is `boundary roles ...`.
...
they always require an `-id` parameter.
...
- `-addr` `(string: "")` - Address of the Boundary controller, as a complete URL
  (as example `https://boundary.example.com:9200`). You can also specify the address
...
- `-ca-cert` `(string: "")` - Path on the local disk to a single PEM-encoded CA
  ...
  This value takes precedence over `-ca-path`. You can also specify the path can also be using the **BOUNDARY_CACERT** environment variable.
...
- `-tls-insecure`  - Disables verification of TLS certificates.
...
- `-keyring-type` `(string: "")` - The type of keyring to use. The default value is
  ...
  keyring functionality. Available keyring types, depending on platform, are: `wincred`,
  `keychain`, `pass`, and `secret-service`. You can also specify the keyring type using the **BOUNDARY_KEYRING_TYPE** environment variable.

- `-output-curl-string`  - Formats the command that
...
- `-recovery-config` `(string: "")` - Specifies a configuration
...
- `-token` `(string: "")` - A URL that points to a file on disk `(file://)` from
  which a token is read or an environment variable `(env://)` from which the token is read. This value overrides the `token-name` parameter.

- `-token-name` `(string: "")` - Designates the name of the token. When the CLI authenticates, it stores the token
  in the platform-specific OS credential store. You can use the `token-name`
  parameter to store more than one token at a time. Specifying this
  parameter during authentication uses the given name as part of the key for
  storage; specifying it for any other command causes the corresponding
  token to be used for that call. You can also specify the token name using the
...
provide the token in subsequent commands.
 ...
 - `-format` `(string: "")` - Prints the output in the given format. Valid
...
Path on the local disk to a single PEM-encoded private key that matches the client certificate
specified by the `BOUNDARY_CLIENT_CERT` variable or the `-client-cert` connection option.

### `BOUNDARY_TLS_INSECURE`

Disables verification of TLS certificates. Using this option is highly discouraged as it
...
OSX keychain, or cross-platform password store, depending on platform.
Set the value to `none` to disable keyring
functionality. Available keyring types, depending on the platform, include: `wincred`, `keychain`, `pass`, and `secret-service`.

### `BOUNDARY_RECOVERY_CONFIG`

Determines if the given config file is parsed for a "kms" block with the purpose "recovery".
If you specify a value, Boundary uses the recovery mechanism to authorize the call.

### `BOUNDARY_TOKEN_NAME`

Designates the token name when stored in the system credential store.
...
Executes the given binary after connecting to the worker.
...
Attempts to bind the listening address to the given value, which must be an IP address.
...
Attempts to bind the listening port to the given value, if you set this variable.
If the CLI cannot bind the address, the command produces an error.

### `BOUNDARY_CONNECT_TARGET_SCOPE_ID`

Target scope ID, if you authorize the session using scope parameters and target name.
This variable is mutually exclusive with `BOUNDARY_CONNECT_TARGET_SCOPE_NAME`.

### `BOUNDARY_CONNECT_TARGET_SCOPE_NAME`

Target scope name, if you authorize the session using scope parameters and target name.
...
Specifies an auth method ID.

### `BOUNDARY_LOG_LEVEL`

Specifies the log verbosity level, mostly as a fallback for events.
...
Specifies the scope to use for the operation.
...
Prints the output in the given format. Valid formats are `table` and `json`. The default value is `table`.
