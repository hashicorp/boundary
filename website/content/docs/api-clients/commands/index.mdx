...
- `-addr` `(string: "")` - Indicates the address of the Boundary controller, as a complete URL, for example `https://boundary.example.com:9200`. You can also specify the address
  using the **BOUNDARY_ADDR** environment variable.

- `-ca-cert` `(string: "")` - Indicates the path on the local disk to a single PEM-encoded CA
...
- `-ca-path` `(string: "")` - Indicates the path on the local disk to a directory of
...
- `-client-cert` `(string: "")` - Indicates the path on the local disk to a single PEM-encoded
...
- `-client-key` `(string: "")` - Indicates the path on the local disk to a single PEM-encoded
...
- `-tls-server-name` `(string: "")` - Specifies the name to use as the SNI host when
...
- `-keyring-type` `(string: "")` - Indicates the type of keyring to use. The default value is
...
- `-token` `(string: "")` - Specifies a URL that points to a file on disk `(file://)` or an environment variable `(env://)` from which the token is read. This value overrides the `token-name` parameter.
...
Indicates the address of the Boundary controller as a complete URL, for example `https://boundary.example.com:9200`.

### `BOUNDARY_CACERT`

Specifies the path on the local disk to a single PEM-encoded CA certificate to verify the controller
...
Specifies the path on the local disk to a directory of PEM-encoded CA certificates to verify the SSL
certificate of the controller.

### `BOUNDARY_CLIENT_CERT`

Specifies the path on the local disk to a single PEM-encoded CA certificate to use for TLS authentication
...
Specifies the path on the local disk to a single PEM-encoded private key that matches the client certificate
...
Indicates the name to use as the SNI host when you connect to the Boundary server using TLS.
...
Specifies the type of keyring to use. Defaults to `auto` which uses the Windows credential manager,
...
Specifies the authorization string returned from the Boundary controller when an "authorize-session" action is used against a target.
...
Indicates the target scope ID, if you authorize the session using scope parameters and target name.
This variable is mutually exclusive with `BOUNDARY_CONNECT_TARGET_SCOPE_NAME`.

### `BOUNDARY_CONNECT_TARGET_SCOPE_NAME`

Indicates the target scope name, if you authorize the session using scope parameters and target name.
