...
flags, and in some cases the parameters to those flags.
...
If you want to install it manually using Bash, add the following line in a
...
- `-addr` `(string: "")` - Address of the Boundary controller, as a complete URL
  (as example `https://boundary.example.com:9200`). This can also be specified
  via the `BOUNDARY_ADDR` environment variable.

- `-ca-cert` `(string: "")` - Path on the local disk to a single PEM-encoded CA
  certificate to verify the Controller or Worker's server's SSL certificate.
  This takes precedence over `-ca-path`. This can also be specified via the
  `BOUNDARY_CACERT` environment variable.

- `-ca-path` `(string: "")` - Path on the local disk to a directory of
  PEM-encoded CA certificates to verify the SSL certificate of the Controller.
  This can also be specified via the `BOUNDARY_CAPATH` environment variable.

- `-client-cert` `(string: "")` - Path on the local disk to a single PEM-encoded
  CA certificate to use for TLS authentication to the Boundary Controller. If
  this flag is specified, `-client-key` is also required. This can also be
  specified via the `BOUNDARY_CLIENT_CERT` environment variable.

- `-client-key` `(string: "")` - Path on the local disk to a single PEM-encoded
...
- `-tls-server-name` `(string: "")` - Name to use as the SNI host when
  connecting to the Boundary server via TLS. This can also be specified via the
...
- `-keyring-type` `(string: "")` - The type of keyring to use. Defaults to
  ...
  keyring functionality. Available types, depending on platform, are: `wincred`,
...
- `-recovery-config` `(string: "")` - This is used to specify a configuration
  file that contains the information necessary to access a KMS configured to be
  used for the recovery workflow within a Boundary controller. This can also be
  specified via the `BOUNDARY_RECOVERY_CONFIG` environment variable.

- `-token` `(string: "")` - A URL pointing to a file on disk `(file://)` from
  which a token will be read or an env var `(env://)` from which the token will
  be read. Overrides the `token-name` parameter.

- `-token-name` `(string: "")` - When the CLI authenticates, it stores the token
  in the platform-specific OS credential store. By using the `token-name`
  parameter, more than one token can be stored at a time. Specifying this
  parameter at authentication time uses the given name as part of the key for
  storage; specifying it for any other command will cause the corresponding
  token to be used for that call. This can also be specified via the
...
provide the token in subsequent commands.
 ...
 - `-format` `(string: "")` - Print the output in the given format. Valid
   formats are `table` or `json`. The default is `table`. This can also be
   specified via the `BOUNDARY_CLI_FORMAT` environment variable.
...
Address of the Boundary controller, as a complete URL (e.g. https://boundary.example.com:9200)

### `BOUNDARY_CACERT`

Path on the local disk to a single PEM-encoded CA certificate to verify the Controller
or Worker's SSL certificate. This takes precedence over `BOUNDARY_CAPATH` variable or
-ca-path connection option, if specified.

### `BOUNDARY_CAPATH`

Path on the local disk to a directory of PEM-encoded CA certificates to verify the SSL
certificate of the Controller.

### `BOUNDARY_CLIENT_CERT`

Path on the local disk to a single PEM-encoded CA certificate to use for TLS authentication
to the Boundary Controller. If set, client key must also be specified with
`BOUNDARY_CLIENT_KEY` variable or -client-key connection option.

### `BOUNDARY_CLIENT_KEY`

Path on the local disk to a single PEM-encoded private key matching the client certificate
specified by `BOUNDARY_CLIENT_CERT` variable or -client-cert connection option.

### `BOUNDARY_TLS_INSECURE`

Disable verification of TLS certificates. Using this option is highly discouraged as it
decreases the security of data transmissions to and from the Boundary server. The default is
false.

### `BOUNDARY_TLS_SERVER_NAME`

Name to use as the SNI host when connecting to the Boundary server via TLS. 

### Client Options

### `BOUNDARY_KEYRING_TYPE`

The type of keyring to use. Defaults to "auto" which will use the Windows credential manager,
OSX keychain, or cross-platform password store depending on platform. Set to "none" to disable keyring
functionality. Available types, depending on platform, are: "wincred", "keychain", "pass", and
"secret-service". The default is auto.

### `BOUNDARY_RECOVERY_CONFIG`

If specified, the given config file will be parsed for a "kms" block with purpose "recovery" and will
use the recovery mechanism to authorize the call.

### `BOUNDARY_TOKEN_NAME`

If specified, the given value will be used as the name when storing the token in the system credential store.
This can allow switching user identities for different commands.

### Connect Options

### `BOUNDARY_CONNECT_AUTHZ_TOKEN`

The authorization string returned from the Boundary controller via an "authorize-session" action
against a target. If set to "-", the command will attempt to read in the authorization string from standard input.

### `BOUNDARY_CONNECT_EXEC`

If set, after connecting to the worker, the given binary will be executed. This should be a binary on your path, or
an absolute path. If all command flags are followed by " -- " (space, two hyphens, space), then any arguments after
that will be sent directly to the binary.

### `BOUNDARY_CONNECT_LISTEN_ADDR`

If set, the CLI will attempt to bind its listening address to the given value, which must be an IP address. If it cannot,
the command will error. If not set, defaults to the most common IPv4 loopback address (127.0.0.1).

### `BOUNDARY_CONNECT_LISTEN_PORT`

If set, the CLI will attempt to bind its listening port to the given value. If it cannot, the command will error.

### `BOUNDARY_CONNECT_TARGET_SCOPE_ID`

Target scope ID, if authorizing the session via scope parameters and target name. Mutually exclusive with `BOUNDARY_CONNECT_TARGET_SCOPE_NAME`

### `BOUNDARY_CONNECT_TARGET_SCOPE_NAME`

Target scope name, if authorizing the session via scope parameters and target name. Mutually exclusive with `BOUNDARY_CONNECT_TARGET_SCOPE_ID`

### Command Options

### `BOUNDARY_AUTH_METHOD_ID`

Specify an auth method ID

### `BOUNDARY_LOG_LEVEL`

Log verbosity level, mostly as a fallback for events. Supported values (in order of more detail to less) are
...
Print the output in the given format. Valid formats are "table" or "json". The default is table.
