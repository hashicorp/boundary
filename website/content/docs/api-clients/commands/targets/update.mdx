...
- `-address=<string>` - Indicates an optional valid network address for the target to connect to.
You cannot use an address alongside host sources.
- `-default-client-port=<string>` - Indicates the default client port on the target, if set.
- `-default-port=<string>` - Indicates the default port on the target, if set.
...
- `-enable-session-recording=<string>` - Speciffies a Boolean expression you can use to enable session recording for the target.
...
- `-storage-bucket-id=<string>` - Indicates the public ID of the storage bucket you want to associate with the target.
