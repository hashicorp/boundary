...
- `-brokered-credential-source=<string>` - Indicates the brokered credential source to set on the target.
...
- `-id=<string>` - Indicates the ID of the target you want to set the credential source on.
- `-injected-application-credential-source=<string>` - Indicates the injected credential source to set on the target.
Boundary injects the credential source when it creates a connection.
You may specify multiple credential sources.
- `-version=<int>` - Specifies the version of the target to perform the operation on.
