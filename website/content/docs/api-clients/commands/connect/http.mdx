...
- `-host` `(string: "")` - Specifies the host value to use and overides the
  endpoint address from the session information.
  Boundary passes the specified hostname through to the client for use in the host header and TLS SNI value, if supported.
...
- `-path` `(string: "")` - Specifies the path that will be appended to the
  generated URL.
...
- `-style` `(string: "")` - Specifies how the CLI attempts to invoke an HTTP client.
