...
[`-listen-addr`](/boundary/docs/api-clients/commands/connect#-listen-port) and
[`-listen-port`](/boundary/docs/api-clients/commands/connect#-listen-addr) flags
...
- [http](/boundary/docs/api-clients/commands/connect/http)
- [kube](/boundary/docs/api-clients/commands/connect/kube)
- [postgres](/boundary/docs/api-clients/commands/connect/postgres)
- [rdp](/boundary/docs/api-clients/commands/connect/rdp)
- [ssh](/boundary/docs/api-clients/commands/connect/ssh)

### Command options

-  `-addr` `(string: "")` - Indicates the address of the Boundary controller as a complete URL, for example https://boundary.example.com:9200.
...
-  `-ca-cert` `(string: "")` - Indicates the path on the local disk to a single PEM-encoded CA certificate that should be used to verify the controller or worker server's SSL certificate.
   This value takes precedence over `-ca-path`.
   You can also specify the path using the **BOUNDARY_CACERT** environment variable.
-  `-ca-path` `(string: "")` - Indicates the path on the local disk to a directory of PEM-encoded CA certificates that should be used to verify the controller's SSL certificate.
   You can also specify the path using the **BOUNDARY_CAPATH** environment variable.
-  `-client-cert` `(string: "")` - Indicates the path on the local disk to a single PEM-encoded CA certificate that should be used for TLS authentication to the Boundary controller.
...
-  `-client-key` `(string: "")` - Indicates the path on the local disk to a single PEM-encoded private key that matches the client certificate you referenced with `-client-cert`.
  You can also specify the path using the **BOUNDARY_CLIENT_KEY** environment variable.
-  `-listen-addr` `(string: "")` - Determines whether the CLI attempts to bind its listening address to the given value, which must be an IP address.
...
-  `-listen-port` `(string: "")` - Determines whether the CLI attempts to bind its listening port to the given value.
...
-  `-tls-server-name` `(string: "")` - Designates a name to use as the SNI host when you connect to the Boundary server using TLS.
