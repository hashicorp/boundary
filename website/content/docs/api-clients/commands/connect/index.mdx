...
the Boundary helper for SSH taht is automatically accept hosts SSH key prompts for you.
...
The following shows use of the [`-listen-addr`] and [`-listen-port`] (/boundary/docs/api-clients/commands/connect#-listen-addr) flag to specify the listening address and port used that Boundary will attemt to use for every connect session. For this example we have used the `127.0.0.1` and port `8872`.
```shell-session
