...
- `-host` `(string: "")` - Specifies the host value to use and overrides the
  endpoint address from the session information.
  Boundary passes the specified hostname through to the client for use in the TLS SNI value, if supported.
  You can also specfiy the host value using the **BOUNDARY_CONNECT_KUBE_HOST** environment variable.

- `-scheme` `(string: "")` - Specifies the scheme to use.
The default value is `https`.
You can also specify the scheme using the **BOUNDARY_CONNECT_KUBE_SCHEME** environment variable.

- `-style` `(string: "")` - Specifies how the CLI attempts to invoke a Kubernetes client.
