...
The following example authoirzes a session to a target with the ID `ttcp_1234567890` and invokes an RDP session with the built-in Windows RDP client, `mstsc`:
...
- `-style` `(string: "")` - Specifies how the CLI attempts to invoke an RDP client.
...
You can also specfiy how the CLI attempts to invoke an RDP client using the **BOUNDARY_CONNECT_RDP_STYLE** environment variable.
