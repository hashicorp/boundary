...
If you use flags ato skip any of these resources, any resources that would be created afterwards are also skipped.
...
- `-config-kms` `(string: "")` - Indicates the path to a configuration file containing a "kms" block marked for tje "config" purpose.
...
- `-log-format` `(string: "")` - Specifies the log format. Supported values are `standard` and `json`.
...
Configuring this option can allow different permissions for the user running initialization vs. normal operation.
