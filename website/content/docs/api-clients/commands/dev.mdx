...
- `-api-listen-address` `(string: "")` - Indicates the address to bind for controller "api"
...
- `-audit-events` `(string: "")` - Indicates whether you want to emit audit events. Supported values are `true` and `false`.

- `-bsr-key` `(string: "")` - Designates a valid, base64-encoded AES key to be used
   for session recording. You can also specify the BSR key using the **BOUNDARY_DEV_BSR_KEY**
   environment variable.

- `-cluster-listen-address` `(string: "")` - Indicates the address to bind for controller
...
- `-combine-logs`  - Sends both startup information and logs to stdout.
   If you do not set this value, startup information goes
   to stdout and logs are sent to stderr. The default is `false`.

- `-container-image` `(string: "")` - Specifies a container image to use.
   This value must be in `<repo>:<tag>` format

- `-controller-only`  - Indicates that only a dev controller should be
   started instead of both a dev controller and dev worker. The default value is `false`.

- `-controller-public-cluster-address` `(string: "")` - Indicates the public address at which
...
- `-database-url` `(string: "")` - Specifies the URL that Boundary uses to connect to
...
- `-disable-database-destruction`  - Creates a database automatically in Docker.
   The database is not removed when the dev server is shut down.
   The default value is `false`.

- `-event-allow-filter` `(string: "")` - Designates an optional allow filter for every event.
  You can specfiy this value multiple times.

- `-event-deny-filter` `(string: "")` - Designates an optional deny filter for every event.
  You can specify this value multiple times.

- `-event-format` `(string: "")` - Indicates the event format.
...
- `-host-address` `(string: "")` - Indicates the address to use for the default host that is
...
- `-id-suffix` `(string: "")` - Designates that auto-created resources use this
...
- `-log-format` `(string: "")` - Designates the log format. Supported values are `standard` and
  `json`.

- `-log-level` `(string: "")` - Indicates the desired log verbosity level.
...
- `-login-name` `(string: "")` - Designates the initial admin login name. If you set this value to an empty
...
- `-observation-events` `(string: "")` - Indicates whether you want Boundary to emit observation events. Supported values are `true` and `false`.

- `-ops-listen-address` `(string: "")` - Indicates an address to bind to for "ops" purpose.
...
- `-password` `(string: "")` - Indicates the initial admin login password. If you set this value to an empty
...
- `-plugin-execution-dir` `(string: "")` - Specifies the directory where Boundary should write
...
- `-proxy-listen-address` `(string: "")` - Indicates the address to bind for worker "proxy"
...
- `-secondary-id-suffix` `(string: "")` - Indicates that secondary auto-created
...
- `-system-events` `(string: "")` - Indicates whether you want Boundary to emit system events. Supported values are `true` and `false`.

- `-target-default-port` `(int: 0)` - Indicates the default port to use for the default target
...
- `-target-session-connection-limit` `(int: 0)` - Indicates the maximum number of connections
...
- `-target-session-max-seconds` `(int: 0)` - Indicates the maximum number of seconds to use for sessions on the default target.
...
- `-unprivileged-login-name` `(string: "")` - Indicates the initial unprivileged user's name. If
...
- `-unprivileged-password` `(string: "")` - Indicates the initial unprivileged user login
...
- `-worker-auth-enable-debugging`  - Turns on debug logging for the
   worker authentication process. The default value is `false`.

- `-worker-auth-key` `(string: "")` - Designates a valid, base64-encoded AES key to
   use for worker-auth purposes You can also specify a key using the
   **BOUNDARY_DEV_WORKER_AUTH_KEY** environment variable.

- `-worker-auth-method` `(string: "")` - Lets you specify how the generated
   worker authenticates to the controller. The default is `random`.

- `-worker-auth-storage-dir` `(string: "")` - Specifies the directory in which to store
...
- `-worker-public-address` `(string: "")` - Indicates the public address at which the worker
...
- `-worker-recording-storage-dir` `(string: "")` - Specifies the directory in which to
