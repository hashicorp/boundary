...
- `-api-listen-address` `(string: "")` - Address to bind for controller "api"
...
- `-bsr-key` `(string: "")` - A valid, base64-encoded AES key that is used
   for session recording. You can also specify the BSR key using the **BOUNDARY_DEV_BSR_KEY**
   environment variable.

- `-cluster-listen-address` `(string: "")` - Address to bind for controller
...
- `-id-suffix` `(string: "")` - Designates that auto-created resources use this
   ...
   generated TCP target ID is "ttcp_1234567890", and so on. This value must be
   different from any `-secondary-id-suffix` or **BOUNDARY_DEV_SECONDARY_ID_SUFFIX** value.
...
- `-proxy-listen-address` `(string: "")` - Indicates the address to bind to for worker "proxy"
