...
- `-config-kms` `(string: "")` - Indicates the path to a configuration file that contains a "kms"
...
- `-log-format` `(string: "")` - Indicates the log format to use, mostly as a fallback for events.
   Supported values are `standard` and `json`.

- `-log-level` `(string: "")` - Indicates the log verbosity level to use, mostly as a fallback for
