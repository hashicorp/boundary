...
- `-config-kms` `(string: "")` - Path to a configuration file that contains a "kms"
   block marked for "config" purpose.
   The kms block decrypts the main configuration file.
   If you do not set a kms block, Boundary looks for such a block in the main
...
- `-log-level` `(string: "")` - Log verbosity level, mostly as a fallback for
   events. Supported values are "trace", "debug", "info", "warn", and "err", in order of more detail to less.
