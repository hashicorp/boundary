...
- `-keyring-type` `(string: "")` - The type of keyring to use.
   ...
   Depending on the platform, the following types of keyring are also supported: `wincred`, `keychain`, `pass`, and `secret-service`.
...
- `-token-name` `(string: "")` - The name to use when Boundary loads the token from the system credential store.
   ...
   You can also be specfiy a token name using the **BOUNDARY_TOKEN_NAME** environment variable.