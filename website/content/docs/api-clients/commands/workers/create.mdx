...
The `boundary workers create controller-led` command lets you create a worker by receiving an activation token from a controller.
...
- `-description=<string>` - Description to set on the controller-led type worker.
- `name=<string>` - Name to set on the controller-led type worker.
- `scope-id=<string>` - Scope in which to make the request.
...
The `boundary workers create worker-led` command lets you create a worker by providing an auth method from the worker.
...
- `-description=<string>` - Description to set on the worker-led type worker.
- `name=<string>` - Name to set on the worker-led type worker.
- `scope-id=<string>` - Scope in which to make the request.
The default is `global`.
You can also specify the scope using the BOUNDARY_SCOPE_ID environment variable.
...
- `worker-generated-auth-token=<string>` - The auth token provided by the worker that is used to register the worker with Boundary.
