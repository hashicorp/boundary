...
- `-description=<string>` - Sets a description on the controller-led type worker.
- `name=<string>` - Sets a name on the controller-led type worker.
- `scope-id=<string>` - Specifies the scope in which to make the request.
...
- `-description=<string>` - Sets a description on the worker-led type worker.
- `name=<string>` - Sets a name on the worker-led type worker.
- `scope-id=<string>` - Specifies the scope in which to make the request.
The default is `global`.
You can also specify the scope using the **BOUNDARY_SCOPE_ID** environment variable.
...
- `worker-generated-auth-token=<string>` - Specifies the auth token that is provided by the worker and used to register the worker with Boundary.
