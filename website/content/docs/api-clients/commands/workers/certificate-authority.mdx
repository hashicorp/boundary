...
- `-scope-id=<string>` - Indicates the scope in which to read the certificate authority.
...
- `-scope-id=<string>` - Indicates the scope in which to reinitialize the certificate authority.
The default scope is `global`.
You can also specify the scope using the **BOUNDARY_SCOPE_ID** environment variable.
