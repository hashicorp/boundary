...
The `boundary workers certificate-authority read` command lets you read the certificate authority that is used to authorize Boundary workers.
...
- `-scope-id=<string>` - Scope in which to read the certificate authority.
...
The `boundary workers certificate-authority reinitialize` command lets you reinitialize the certificate authority that is used to authorize Boundary workers.

#### Usage

```shell-session
$ boundary workers certificate-authority reinitialize [options] [args]
```

#### Worker certificate authority options

- `-scope-id=<string>` - Scope in which to reinitialize the certificate authority.
The default scope is `global`.
You can also specify the scope using the BOUNDARY_SCOPE_ID environment variable.
