...
## Vault as a key manager

Boundary supports various KMS systems that protect the base encryption keys used for various functions. Boundary can use the Vault transit secrets engine as one such KMS system.
...
### Boundary and Vault transit key lifecycle management

The Vault transit secrets engine supports both automatic and manual rotation of configured keys.

The Vault transit secrets engine can be configured with [`auto_rotate_period`](https://developer.hashicorp.com/vault/api-docs/secret/transit#auto_rotate_period) to configure
automatic rotation of the configured keys. Boundary will accommodate the automatic rotation of keys in the backend.

Keys can also be [manually rotated](/boundary/docs/concepts/security/data-encryption#key-version-lifecycle-management) from the Boundary API or CLI.