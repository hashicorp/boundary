...
`Ctrl-C/Command-C` to the `boundary connect` process to shut down the session.
...
for you.
These are `boundary connect <subcommand>` and support the following connection types:
