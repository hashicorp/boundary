...
1. **Bring your own identity.**
   We feel strongly that Boundary’s identity-based controls should use the same identity that users have for their other applications. To do so, we’ll progressively add support for new authentication methods for Boundary. Our first step will be in delivering an OpenID Connect (OIDC) authentication method.
