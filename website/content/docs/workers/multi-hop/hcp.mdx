...
# Configure multi-hop sessions for HCP
...
- Include a [worker tag](/boundary/docs/workers/worker-tags#target-worker-filtering) in the `worker` stanza which will be used to select multi-hop routes for each target.
