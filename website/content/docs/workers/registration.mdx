...
routing to the correct target, refer to [Use worker tags to control access](/boundary/docs/workers/worker-tags).
...
The `name` and `description` fields are not valid configuration fields for workers that use worker-led or controller-led authorization. You can only set these fields through the API and they are only valid for workers that use the [KMS authorization method](/boundary/docs/workers/configure-workers/registration#kms-led-authorization-authentication-flow).
