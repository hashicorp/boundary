...
## Signals
On `SIGHUP`, worker and controller processes will reload their configuration file to pick up updated values

~> **Note:** Not all configuration values can be reloaded. Please see the configuration pages for
...
On `SIGTERM` or `SIGINT`, worker and controller processes will enter graceful shutdown. Controller graceful shutdown
will close listeners and servers before shutting down. Worker graceful shutdown will wait for sessions to drain
before shutting down. Workers in a graceful shutdown state will not receive new work, including session proxying, from the control plane.
