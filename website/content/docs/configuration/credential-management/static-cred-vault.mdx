...
1. Complete the fields related to your Vault instance:
    ...
    - **Worker Filter** (optional) - If your Vault instance does not have a publicly accessible address and instead is proxied through a Boundary worker, enter the worker filter. This should be a boolean expression. Refer to the examples in the [Worker tags](/docs/concepts/filtering/worker-tags) documentation.
...
- You must have a static credential saved in the Vault. The static credential must correspond to the target to which you want to authenticate.
...
- You must have an existing target available.
- You must have a Vault credential store and credential library. Refer to [Create Vault credential stores](/boundary/docs/configuration/credential-management/static-cred-vault#create-a-vault-credential-store) page for procedures.
- You must have a static credential saved in the Vault credential store. The static credential must correspond to the target to which you want to authenticate.
