...
## Signals
On `SIGHUP`, a worker will reload its configuration file to pick up updated values for `initial_upstreams` and `tags`.
All other updated values will be ignored.

`SIGTERM` or `SIGINT` will initiate 'graceful shutdown' on a worker. The worker will wait for sessions to drain
