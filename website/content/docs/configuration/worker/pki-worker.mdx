...
Boundary cluster or the `initial_upstreams` field to connect directly to other workers,
 when set to the worker's `public_addr` field.
...
Inbound session connections from clients reach the egress worker via reverse proxy; the ingress worker is initatied from the egress worker.
