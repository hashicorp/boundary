...
above a worker in the chain. For example, int the diagram below, Worker2’s upstream is Worker1, and its
downstream is Worker3.
...
You can configure [target worker filters][] can be used with multi-hop workers to allow for fine grained control on which workers handle
