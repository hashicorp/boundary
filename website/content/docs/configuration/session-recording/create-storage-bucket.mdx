...
The worker configured for storage must:
...
Complete the following steps to create a storage bucket in Boundary for session recording. The steps require you to select Static or Dynamic credentials.
...
The required fields for creating a storage bucket depend on whether you configured the AWS S3 bucket with static or dynamic credentials.
...
After creating the storage bucket resource Boundary provides you with the bucket's ID.
