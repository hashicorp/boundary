...
- An AWS IAM role policy with the following statement:
   ...
   ```
- If KMS encryption is applied to the storage bucket, you will need to add these additional permissions for the Storage Bucket IAM User to the role policy:
   ```json
   {
      "Action": [
      "kms:Decrypt",
