...
When you [create a storage bucket](/docs/configuration/session-recording/create-storage-bucket), Boundary provides you with an ID.
...
1. Do one of the following:
   ...
   - To enable an existing SSH target for session recording, run the following commmand:
      ...
      ```hcl
      boundary targets update   -scope-id p_1234567890 -enable-session-recording true -storage-bucket-id 123
      ```
   ...
   - To create a new target and enable it for session recording, run the following command:
      ...
      ```hcl
      boundary targets create ssh -scope-id p_1234567890 -default -port 22 -name test1 -address 99.12.345.67 -enable-session-recording true -storage-bucket-id 1234
      ```

      Make sure to add the `-enable-session-recording true` flag to turn on session recording for the target.
      Add the `-storage-bucket-id ID` for the storage bucket you want to associate with this target.
      You can configure any other [target attributes](/docs/concepts/domain-model/targets).
...
1. Configure the target with any relevant [attributes](/docs/concepts/domain-model/targets).
...
1. Click **Save**.
1. Click **Enable Session Recording**.
