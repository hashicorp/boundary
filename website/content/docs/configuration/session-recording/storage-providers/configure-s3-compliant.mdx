...
The addition of the [MinIO plugin](https://github.com/hashicorp/boundary-plugin-minio/) in Boundary version 0.16.0 made it possible for you to [configure MinIO storage](/boundary/docs/configuration/session-recording/storage-providers/configure-minio) for session recording if you do not want to use AWS.
It also means that you should be able to configure other S3-compliant storage providers for session recording.

HashiCorp has tested and confirmed that you can configure the Hitachi Content Platform for session recording using the MinIO plugin.
It should also be possible to configure other S3-compliant storage providers to work for session recording storage, but we have not tested other providers.
