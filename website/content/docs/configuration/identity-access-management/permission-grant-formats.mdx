...
By default, the following grants are automatically created for HCP Boundary clusters:

- The default admin role `global_admin_role` is set to `descendants`.
- The default login role `global_anon_listing` is set to `descendants` and includes the following grant:
   ```
   ids=*,type=auth-token;actions=read:self,delete:self
   ```
- Authenticated users `u_auth` have a role on the `global` scope and are set to `descendants` with the following grants:
