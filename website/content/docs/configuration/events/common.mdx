...
- `allow_filters` - Specifies a set predicates for including an event in the
  sink. If any filter matches, the event will be included. The filter should be
  in a format supported by hashicorp/go-bexpr.
  
- `deny-filters` - Specifies a set predicates for excluding an event in the
  sink. If any filter matches, the event will be excluded. The filter should be
  in a format supported by hashicorp/go-bexpr.
