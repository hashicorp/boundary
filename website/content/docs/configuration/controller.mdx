...
## Signals
On `SIGHUP`, a controller will reload its configuration file to pick up any updates to its `database url`. All other updated values will be ignored.

`SIGTERM` or `SIGINT` will initiate graceful shutdown on a controller. Controller graceful shutdown
will close listeners and servers before shutting down.
