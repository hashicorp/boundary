...
- `public_cluster_addr` - Specifies the public host or IP address (and
  ...
  Then the controller gives the worker the `public_cluster_addr` and the worker updates its client-side load balancing configuration to reach out to the set of cluster addresses.
