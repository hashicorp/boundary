...
- `database` - Configuration block with two valid parameters for connecting to Postgres:
  ...
  - `max_open_connections` - Can be used to control the maximum number of connections that can be opened by the controller.
  The minimum number of connections required is 2. Setting this value to 0 will allow the controller to open as many connections as 
  needed.
