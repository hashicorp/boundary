...
The RDP helper will attempt to determine which RDP client to use
depending on your operating system. On Windows, it will use `mstsc.exe`. On
Mac, it will use `open`. Other operating systems are not currently supported.

```shell-session
$ boundary connect rdp -target-id ttcp_eTcZMueUYv
```

This will execute the RDP client and proxy the connection through
Boundary.
...
The PostgreSQL helper executes `psql`, passing information such as the username and
...
The PostgreSQL helper will automatically inject brokered credentials, allowing you
...
- `method`: Specifies the method to use. If not set, will use the client's default.

- `path`: Specifies a path that will be appended to the generated URL.
