...
- `{{boundary.password}}` (`BOUNDARY_PROXIED_PASSWORD`): if an available credential
  library is attached to the target, the first brokered password provided by the credential store.
- `{{boundary.username}}` (`BOUNDARY_PROXIED_USERNAME`): if an available credential
  library is attached to the target, the first brokered username provided by the credential store.
