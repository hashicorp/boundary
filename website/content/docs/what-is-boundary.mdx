...
- **Extensibility with the Ecosystem:** While modern organizations can require a multilayered matrix of identity providers, policy engines, secrets management tools, target types, and cloud  providers to integrate and reside within their access workflow, there is no vendor lock-in requirement with Boundary; we support the user's vendor-of-choice.

![Boundary workflow](/img/boundary.png)

## How does Boundary work?

While the more traditional approach for users to access their resources may require that organizations implement and maintain SSH bastion hosts and VPNs, Boundary provides a secure way to access hosts and critical systems without having to distribute and manage credentials, configure network controls like firewalls, or expose the organization's private network. The illustration below displays Boundary's core workflow.

![Boundary core workflow](/img/boundary-core-workflow.png)

The core Boundary worklow consists of four stages:
...
- **Granula Authorization:** Boundary authenticates and authorizes users based on their roles and logical services, and tightly controls access and actions performed against systems.
...
The key features and concepts of Boundary are:
...
Boundary offers two types of deployment options. The first option is the *self-managed* approach, which we have discussed on this page. The self-managed approach enables organizations to proxy all session data through their own network while still providing the convenience of a managed services. HCP Boundary, on the other hand,  is a *fully-managed*, cloud-based workflow that enables secure connections to remote hosts and critical systems across cloud and on-premise environments. In the fully-managed deployment model, both the control plan and worker nodes are managed by HashiCorp. Refer to the [HCP Boundary](https://cloud.hashicorp.com/docs/boundary
) documentation to learn more.

> **Hands On:** Try the [Create a Boundary Instance on HCP](https://learn.hashicorp.com/tutorials/boundary/hcp-getting-started-create?in=boundary/hcp-getting-started) tutorial on HashiCorp Learn to deploy a Boundary instance.
...
- [Submit an issue](https://support.hashicorp.com/hc/en-us/requests/new) for bugs and feature requests.
