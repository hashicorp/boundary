...
Welcome to Boundary! This introduction illustrates some of the challenges Boundary meets head-on and how we compare with other software solutions. But, if you wish to dive headfirst into Boundary, head over to the [Getting Started](/docs/getting-started) section.
...
**A Consistent Workflow for Access**: Boundary connects the user securely to their infrastructure regardless of the cloud platform, environment on which a target resides, or the identity provider managing users. Boundary creates a consistent workflow for authenticating and authorizing user access to create secure, ephemeral sessions that can be monitored and managed by administrators.

![Boundary workflow](/img/boundary.png)

**Extensibility with the Ecosystem**: Modern organizations have a multilayered matrix of identity providers, policy engines, secrets management tools, target types, and cloud providers that must integrate and reside within the access workflow. Boundary supports an extensible ecosystem that supports users’ vendor-of-choice in this access workflow.
...
Boundary provides session monitoring capabilities that give security administrators visibility into user access. Boundary sessions are logged and consumable via the administrator UI, business intelligence (BI), and security information and event management (SIEM) tools
