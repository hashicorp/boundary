...
1. Complete the following fields:
   ...
   - **Type**: Select the target type.
    You can create RDP (BETA), SSH, or TCP targets.
   - **Target Address**: (Optional) Enter a valid IP address or DNS name.
    Alternatively, you can configure host catalogs and host sets.

    If you set a target address for RDP targets that use Kerberos authentication, set the target address to the target name within the domain, with the domain appended to it, for example `my-target.mydomain.com`. For RDP targets that use NTLM authentication, you can use the IP address.
...
1. Use the following command to create a target:
   ...
   - `-address=<string>` - An optional valid network address for the target to connect to.
     ...
     If you set a target address for RDP targets that use Kerberos authentication, set the target address to the target name within the domain, with the domain appended to it, for example `my-target.mydomain.com`.
     For RDP targets that use NTLM authentication, you can use the IP address.
...
A value of `-1` means the connections are unlimited.
   ...
   - `-with-alias-value=<string>` - The value of the alias that you want to use to represent the target.
     ...
     You can create RDP (BETA), SSH, or TCP [target types](/boundary/docs/domain-model/targets#target-types).
...
If you set a target address for RDP targets (BETA) that use Kerberos authentication, set the target address to the target name within the domain, with the domain appended to it, for example `my-target.mydomain.com`. For RDP targets that use NTLM authentication, you can use the IP address.
