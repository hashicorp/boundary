...
- Include a [worker tag](/boundary/docs/secure/worker-tags#target-worker-filtering) in the `worker` stanza which will be used to select multi-hop routes for each target.
