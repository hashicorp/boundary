...
Refer to [Create the controller configuration](/boundary/docs/deploy/self-managed/configure-controllers#create-the-controller-configuration) and [the `bsr` KMS key](/boundary/docs/security/encryption/data-encryption#the-bsr-kms-key) documentation for more information about configuring a KMS block.
