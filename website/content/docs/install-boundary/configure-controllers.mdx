...
- &#32`/etc/boundary.d/tls/boundary-cert.pem`
...
- &#32`/etc/boundary.d/tls/boundary-key.pem`
...
The time and nonce are used to ensure that a value cannot be replayed by an adversary, and also to ensure that each operation must be individually authenticated by a client, sot hat revoking access to the KMS has an immediate result.
...
Refer to [Data security in Boundary](/docs/concepts/security/data-encryption) for more information.
...
Refer to the [Database recommendations](/docs/install-boundary/system-requirements#database-recommendations) documentation for examples of cloud managed PostgreSQL databases.
...
- &#32 `listener` blocks for `api`, `cluster`, and `ops`
- A `kms` block
- &#32 `disable_mlock`
- A `controller` block
...
1. &#32 `sudo mv boundary.hcl boundary.hcl.old`
1. &#32 `sudo mv controller.hcl controller.hcl.old`
1. &#32 `sudo mv worker.hcl worker.hcl.old`
...
- &#32; `kms` - Configures KMS blocks for [various purposes](/docs/concepts/security/data-encryption).

   Refer to the links below for configuration information for the different cloud KMS blocks:

   - [AWS](/docs/configuration/kms/awskms)
   - [Azure](/docs/configuration/kms/azurekeyvault)
   - [GCP](/docs/configuration/kms/gcpckms)
   - [OCI](/docs/configuration/kms/ocikms)
   - [AliCloud](/docs/configuration/kms/alicloudkms)
   - [Vault Transit](/docs/configuration/kms/transit)

Refer to the documentation for additional [top-level configuration options](/docs/configuration) and additional [controller-specific options](/docs/configuration/controller).
