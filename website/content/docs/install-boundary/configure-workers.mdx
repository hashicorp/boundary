...
Note that in a multi-hop configuration, the Boundary workers can serve one of three purposes: an ingress worker, an ingress/egress worker, or an egress worker.
...
After you create the requisite key or keys in the cloud provider of your choice, you can begin configuring workers.

The following configuration example employs the PKI method of authentication with a worker-led authorization flow.
...
Note that in a multi-hop session <sup>(HCP/ENT)</sup>, the Boundary workers can serve one of three purposes: an ingress worker, an intermediary worker, or an egress worker.
For more information, refer to [Configure workers for multi-hop sessions](/boundary/docs/enterprise/configure-multi-hop-workers).
