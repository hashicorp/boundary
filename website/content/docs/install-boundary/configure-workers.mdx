...
- Installed Boundary on at least three [controller nodes](/docs/install-boundary/configure-controllers).
...
For more information on configuring KMS authentication for Boundary workers, refer to the [KMS worker configuration documenation](/docs/configuration/worker/kms-worker).
...
1. (Optional) You can use the new PKI workers page to construct the contents of the `pki-worker.hcl` file, if you did not [create the configuration file](/docs/install-boundary/configure-workers#create-the-worker-configurations) as part of the installation process above.
