...
In this scenario, the administrator interfaces solely with the Boundary controllers, ideally through a layer 4 or layer 7 load balancer.
...
Boundary controllers are stateless and do not operate on protocols such as raft, unlike other HashiCorp products like Vault, and therefore can withstand failure scenarios where only one node is accessible.

If deploying Boundary to three availability zones is not possible, you can use the same architecture across one or two availability zones, at the expense of significant reliability risk in case of an availability zone outage.
