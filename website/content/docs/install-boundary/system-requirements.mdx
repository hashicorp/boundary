...
** Multi-hop sessions use reverse proxy tunnels initiated by downstream workers connecting to upstream workers when the downstream worker starts.  The connection flow when a client requests to connect to a target that requires a multi-hop path can be broken down into four steps:

1. The client connects to an ingress worker
2. The ingress worker connects with the next-downstream or egress worker through that worker's established reverse-proxy tunnel
    * This may repeat multiple times if there are multiple worker connections in the middle of the tunnel chain
3. The final downstream egress worker connects to the Boundary target
4. The client can now send data through the tunnel chain

A multi-hop worker acting as a downstream can initiate a tunnel to one or multiple upstream workers, specified in the `initial_upstreams` list in the worker configuration.
