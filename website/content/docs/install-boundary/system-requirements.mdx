...
** Multi-hop session proxying occurs in three steps:

  1. The client connects to an ingress worker.
  1. The ingress worker connects to a downstream or egress worker using a reverse-proxied tunnel.

    When you proxy sessions through multiple workers, the upstream or client-side workers always connect to the downstream or target-side workers using reverse-proxied connections.
    The downstream workers initiate the connections.
