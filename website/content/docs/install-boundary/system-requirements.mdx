...
The following table outlines the network connectivity requirements for Boundary cluster nodes.
If general network egress is restricted, particular attention must be paid to granting outgoing access from the Boundary controllers to any external integration providers, for example OIDC authentication providers, external log handlers, metrics collection, and security and configuration management providers.

| Source | Destination | Destination Port | Protocol | Direction | Purpose |
|---|---|---|---|---|---|
| Client machines | Controller load balancer | 443 | TCP | Incoming | Request distribution |
| Load balancer | Controller servers | 9200 | TCP | Incoming | Boundary API |
| Load Balancer | Controller servers | 9200 | TCP | Incoming | Health checks |
| Worker servers | Controller load balancer | 9201 | TCP | Incoming | Session authorization, credentials, etc. |
| Client machines | Worker servers* | 9202 | TCP | Bidirectional | Session proxying |
| Worker servers | Boundary targets* | various | TCP | Bidirectional | Session proxying |
| Client machines | Ingress worker servers** | 9202 | TCP | Bidirectional | Multi-hop session proxying |
| Ingress worker servers | Egress worker servers** | 9202 | TCP | Incoming | Multi-hop session proxying |
| Egress worker servers | Ingress worker servers** | 9202 | TCP | Outgoing | Multi-hop session proxying |
| Egress worker servers | Boundary targets** | various | TCP | Outgoing | Multi-hop session proxying |
...
If you use a [high availability](/docs/install-boundary/high-availability) (HA) configuration, then the controllers must have access to the PostgreSQL server infrastructure.
