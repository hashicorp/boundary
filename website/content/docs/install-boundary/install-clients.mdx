...
Boundary end users access infrastructure targets using the Boundary Desktop Client app or the Boundary CLI. Enterprise or HCP Boundary users can use transparent sessions to proxy traffic though the Boundary Client Agent <sup>HCP/ENT</sup>.

This guide outlines the required steps to manually install and configure a Boundary client depending on which distribution of Boundary end-users are accessing.

To learn about installing the Boundary binary when deploying Boundary controllers or workers, refer to the [Deploy Boundary](/boundary/docs/install-boundary/deploy) page.
...
<Warning>
  ...
  Before launching the Boundary Installer, ensure that you have uninstalled all previous versions of the Boundary binary and Boundary Desktop Client app.
...
The following components are recommended for end-users:
