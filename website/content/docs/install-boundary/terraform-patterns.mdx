...
This document outlines MVP (Minimum Viable Product) patterns to deploy and maintain Boundary using Terraform.
...
The following code block provides an example of provider configuration using the userpass authentication method. You can use any of the available Boundary authentication methods by specifying the appropriate `auth_method_id`.
...
You can use the following patterns to create [org-level](/boundary/docs/concepts/domain-model/scopes#organizations) and
...
After you create an auth method, you need to add accounts to it and create users to represent the accounts. Users and accounts are different constructs. A [user](/boundary/docs/concepts/domain-model/users) is a "parent" object associated to one or more [accounts](/boundary/docs/concepts/domain-model/accounts) created using a supported auth method.
...
Security best-practices recommend that you use Role-Based Access Control (RBAC) when you make authorization decisions. RBAC is a methodology in which you create a [role](/boundary/docs/concepts/domain-model/roles) that defines the actions that a user is allowed to take, and then assign one or more users to that role.

In Boundary, you can assign users directly to a role, but a better pattern is to put users with equivalent access into [groups](/boundary/docs/concepts/domain-model/groups). You can then assign groups to roles that grant least-privileges to your Boundary environment.
...
This example creates a [managed group](/boundary/docs/concepts/domain-model/managed-groups) that is automatically populated based on an LDAP group called `Engineering`.
...
The following example shows how to create a Boundary static [host catalog](/boundary/docs/concepts/domain-model/host-catalogs) and add a known host to that catalog.
...
Boundary supports multiple types of static credentials and Vault dynamic credentials. A [credential store](/boundary/docs/concepts/domain-model/credential-stores) stores credentials for hosts. Static credential stores can hold username/passwords, JSON tokens, SSH key pairs, and SSH certificates.
...
After you create a credential store, you can create one or more [credentials](/boundary/docs/concepts/domain-model/credentials) in that store.
...
## Session recording <a name='session-recording'></a>
...
For more information, refer to the Terraform provider documentation on [targets](/boundary/docs/resources/target).