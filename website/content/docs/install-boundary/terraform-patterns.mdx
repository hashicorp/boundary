...
include:
- [Basic Terraform Provider Configuration](/boundary/docs/install-boundary/terraform-patterns#basic-terraform-provider-configuration)
- [Scopes](/boundary/docs/install-boundary/terraform-patterns#scopes)
- [Users and auth methods](/boundary/docs/install-boundary/terraform-patterns#users-and-auth-methods)
- [Groups and Role-Based-Access-Control (RBAC)](/boundary/docs/install-boundary/terraform-patterns#groups-and-rbac)
...
- [Session Recording](/boundary/docs/install-boundary/terraform-patterns#session-recording)
- [Targets](/boundary/docs/install-boundary/terraform-patterns#targets)

## Assumptions

- An understanding of [Terraform](/terraform/docs) fundamentals
- An existing Boundary installation. Refer to [Running Boundary in non-dev environments](/boundary/docs/install-boundary/no-gen-resources) to learn about deploying Boundary
...
the accounts.  Keep in mind that users and accounts are different constructs.  A [user](/boundary/docs/concepts/domain-model/users) 
is a "master" object that is associated to one or more [accounts](/boundary/docs/concepts/domain-model/accounts) 
...
authorization decisions.  RBAC is a methodology in which you create a [role](/boundary/docs/concepts/domain-model/roles)
...
equivalent access into [groups](/boundary/docs/concepts/domain-model/groups) and then assign those groups to 
...
diretory.  Here is an example of creating a [managed group](/boundary/docs/concepts/domain-model/managed-groups) that will 
...
The first pattern demonstrates how to create a static Boundary [host catalog](/boundary/docs/concepts/domain-model/host-catalogs) as 
...
Before you can access a host you must add it to a [host set](/boundary/docs/concepts/domain-model/host-sets).
...
using or creating any credentials, you will create a [credential store](/boundary/docs/concepts/domain-model/credential-stores).  
...
After creating a store, you can create one or more [credentials](/boundary/docs/concepts/domain-model/credentials) in that store.  The next few patterns demonstrate static credentials that are managed by Boundary
```hcl
...
For Vault-backed credential stores, you can then create a [credential library](/boundary/docs/concepts/domain-model/credential-libraries) which vends credentials of a specific access level from a specific Vault path. 
...
[Session recording](/boundary/docs/concepts/domain-model/session-recordings) requires that you create a storage policy for your current scope and then 
allocate an AWS S3 bucket to store the recordings.  We will not demonstrate creating the 
actual S3 bucket or the associated IAM policies. See [this document](/boundary/docs/configuration/session-recording/create-storage-bucket) for information on how to configure AWS.
