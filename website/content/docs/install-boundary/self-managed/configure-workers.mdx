...
- Installed Boundary on at least three [controller nodes](/boundary/docs/install-boundary/self-managed/configure-controllers).
...
1. (Optional) You can use the workers page to construct the contents of the `worker.hcl` file, if you did not [create the configuration file](/boundary/docs/install-boundary/self-managed/configure-workers#create-the-worker-configurations) as part of the installation process above.
