---
id: 069a5d79-0fce-42b5-8ae0-541bcc6aff8d
name: Manage roles and permissions with HCP Boundary
short_name: Manage roles and permissions with HCP
...
Users and Groups](/boundary/tutorials/hcp-administration/hcp-manage-users-groups) tutorials.
...
Log in to the CLI as the `tester01` user. In the previous tutorial its password
was defined as `supersecure`. Enter `tester01` when prompted for the login name
and `supersecure` at the `Please enter the password (it will be hidden):`
prompt.
...
Groups](/boundary/tutorials/hcp-administration/hcp-manage-users-groups) tutorial to the newly created
...
Log back into the CLI as the `tester01` user. Enter `supersecure` at the `Please
...
Sessions](/boundary/tutorials/hcp-administration/hcp-manage-sessions) tutorial you will learn about
