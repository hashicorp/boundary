...
- `{{boundary.ip}}` (`BOUNDARY_PROXIED_IP`): Indicates the IP address of the listening
  socket that `boundary connect` has opened.
- `{{boundary.port}}` (`BOUNDARY_PROXIED_PORT`): Indicates the port of the listening
  socket that `boundary connect` has opened.
- `{{boundary.addr}}` (`BOUNDARY_PROXIED_ADDR`): Specifies the host:port format of the
