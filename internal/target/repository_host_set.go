...
func (r *Repository) AddTargetHostSets(ctx context.Context, targetId string, targetVersion uint32, hostSetIds []string, _ ...Option) (Target, []*TargetSet, []*TargetLibrary, error) {
	...
	for _, id := range hostSetIds {
		...
		if err != nil {
			return nil, nil, nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory target host set"))
		}
	...
	_, err = r.writer.DoTx(
		...
		func(reader db.Reader, w db.Writer) error {
			...
			if err := w.CreateItems(ctx, newHostSets, db.NewOplogMsgs(&hostSetsOplogMsgs)); err != nil {
				return errors.WrapDeprecated(err, op, errors.WithMsg("unable to add target host sets"))
			}
			...
			if err != nil {
				return errors.WrapDeprecated(err, op, errors.WithMsg("unable to retrieve current host sets after adds"))
			}
			currentCredLibs, err = fetchLibraries(ctx, reader, targetId)
			if err != nil {
				return errors.WrapDeprecated(err, op, errors.WithMsg("unable to retrieve current credential libraries after adds"))
			}
			return nil
		},
	)
	if err != nil {
		return nil, nil, nil, errors.WrapDeprecated(err, op, errors.WithMsg("error creating sets"))
	}
...
func (r *Repository) DeleteTargeHostSets(ctx context.Context, targetId string, targetVersion uint32, hostSetIds []string, _ ...Option) (int, error) {
	...
	for _, id := range hostSetIds {
		ths, err := NewTargetHostSet(targetId, id)
		if err != nil {
			return db.NoRowsAffected, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory target host set"))
		}
	...
	_, err = r.writer.DoTx(
		...
		func(reader db.Reader, w db.Writer) error {
			...
			if err != nil {
				return errors.WrapDeprecated(err, op, errors.WithMsg("unable to delete target host sets"))
			}
...
func (r *Repository) SetTargetHostSets(ctx context.Context, targetId string, targetVersion uint32, hostSetIds []string, _ ...Option) ([]*TargetSet, []*TargetLibrary, int, error) {
	...
	if err != nil {
		return nil, nil, db.NoRowsAffected, errors.WrapDeprecated(err, op, errors.WithMsg("unable to search for existing target host sets"))
	}
	...
	for _, id := range hostSetIds {
		...
		if err != nil {
			return nil, nil, db.NoRowsAffected, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory target host set"))
		}
	...
	_, err = r.writer.DoTx(
		...
		func(reader db.Reader, w db.Writer) error {
			...
			if len(deleteHostSets) > 0 {
				...
				if err != nil {
					return errors.WrapDeprecated(err, op, errors.WithMsg("unable to delete target host set"))
				}
			...
			currentHostSets, err = fetchSets(ctx, reader, targetId)
			if err != nil {
				return errors.WrapDeprecated(err, op, errors.WithMsg("unable to retrieve current target host sets after set"))
			}
			currentCredLibs, err = fetchLibraries(ctx, reader, targetId)
			if err != nil {
				return errors.WrapDeprecated(err, op, errors.WithMsg("unable to retrieve current target credential libraries after set"))
			}
