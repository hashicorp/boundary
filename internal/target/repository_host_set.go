...
func (r *Repository) AddTargetHostSets(ctx context.Context, targetId string, targetVersion uint32, hostSetIds []string, _ ...Option) (Target, []*TargetSet, []*TargetLibrary, error) {
	...
	if len(hostSetIds) == 0 {
		return nil, nil, nil, errors.New(ctx, errors.InvalidParameter, op, "missing host set ids")
	}
	...
	_, err = r.writer.DoTx(
		...
		func(reader db.Reader, w db.Writer) error {
			...
			if err := w.CreateItems(ctx, newHostSets, db.NewOplogMsgs(&hostSetsOplogMsgs)); err != nil {
				return errors.Wrap(ctx, err, op, errors.WithMsg("unable to add target host sets"))
			}
			...
			if err != nil {
				return errors.Wrap(ctx, err, op, errors.WithMsg("unable to retrieve current host sets after adds"))
			}
			currentCredLibs, err = fetchLibraries(ctx, reader, targetId)
			if err != nil {
				return errors.Wrap(ctx, err, op, errors.WithMsg("unable to retrieve current credential libraries after adds"))
			}
			return nil
		},
	)
...
func (r *Repository) DeleteTargeHostSets(ctx context.Context, targetId string, targetVersion uint32, hostSetIds []string, _ ...Option) (int, error) {
	...
	if len(hostSetIds) == 0 {
		return db.NoRowsAffected, errors.New(ctx, errors.InvalidParameter, op, "missing host set ids")
	}
	...
	_, err = r.writer.DoTx(
		...
		func(reader db.Reader, w db.Writer) error {
			...
			if err != nil {
				return errors.Wrap(ctx, err, op, errors.WithMsg("unable to delete target host sets"))
			}
			if rowsDeleted != len(deleteTargeHostSets) {
				return errors.New(ctx, errors.MultipleRecords, op, fmt.Sprintf("target host sets deleted %d did not match request for %d", rowsDeleted, len(deleteTargeHostSets)))
			}
...
func (r *Repository) SetTargetHostSets(ctx context.Context, targetId string, targetVersion uint32, hostSetIds []string, _ ...Option) ([]*TargetSet, []*TargetLibrary, int, error) {
	...
	if err != nil {
		return nil, nil, db.NoRowsAffected, errors.Wrap(ctx, err, op, errors.WithMsg("unable to search for existing target host sets"))
	}
	...
	_, err = r.writer.DoTx(
		...
		func(reader db.Reader, w db.Writer) error {
			...
			if rowsUpdated != 1 {
				return errors.New(ctx, errors.MultipleRecords, op, fmt.Sprintf("set target host sets: updated target and %d rows updated", rowsUpdated))
			}
			msgs = append(msgs, &targetOplogMsg)
			...
			if len(deleteHostSets) > 0 {
				...
				if err != nil {
					return errors.Wrap(ctx, err, op, errors.WithMsg("unable to delete target host set"))
				}
				if rowsDeleted != len(deleteHostSets) {
					return errors.New(ctx, errors.MultipleRecords, op, fmt.Sprintf("target host sets deleted %d did not match request for %d", rowsDeleted, len(deleteHostSets)))
				}
			...
			currentHostSets, err = fetchSets(ctx, reader, targetId)
			if err != nil {
				return errors.Wrap(ctx, err, op, errors.WithMsg("unable to retrieve current target host sets after set"))
			}
			currentCredLibs, err = fetchLibraries(ctx, reader, targetId)
			if err != nil {
				return errors.Wrap(ctx, err, op, errors.WithMsg("unable to retrieve current target credential libraries after set"))
			}
