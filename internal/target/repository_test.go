...
func TestRepository_DeleteTargetHostSets(t *testing.T) {
	t.Parallel()
	conn, _ := db.TestSetup(t, "postgres")
	rw := db.New(conn)
	wrapper := db.TestWrapper(t)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			deletedRows, err := repo.DeleteTargetHostSets(context.Background(), targetId, targetVersion, deleteHostSets, tt.args.opt...)
			if tt.wantErr {
				assert.Error(err)
				assert.Equal(0, deletedRows)
...
func TestRepository_AddTargetHosts(t *testing.T) {
	t.Parallel()
	conn, _ := db.TestSetup(t, "postgres")
	rw := db.New(conn)
	wrapper := db.TestWrapper(t)
	...
	createHostsFn := func(orgs, projects []string) []string {
		results := []string{}
		for _, publicId := range orgs {
			cats := static.TestCatalogs(t, conn, publicId, 1)
			h := static.TestHosts(t, conn, cats[0].GetPublicId(), 1)
			results = append(results, h[0].PublicId)
		}
		for _, publicId := range projects {
			cats := static.TestCatalogs(t, conn, publicId, 1)
			h := static.TestHosts(t, conn, cats[0].GetPublicId(), 1)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var hostSetIds []string
			origTarget, _, origHosts, err := repo.LookupTarget(context.Background(), projTarget.PublicId)
			require.NoError(err)
			require.Equal(0, len(origHosts))

			if tt.args.wantTargetIds {
				hostSetIds = createHostsFn([]string{staticOrg.PublicId}, []string{staticProj.PublicId})
			}

			gotTarget, _, gotHosts, err := repo.AddTargetHosts(context.Background(), projTarget.PublicId, tt.args.targetVersion, hostSetIds, tt.args.opt...)
			if tt.wantErr {
				require.Error(err)
				if tt.wantErrIs != nil {
					assert.Truef(errors.Is(err, tt.wantErrIs), "unexpected error %s", err.Error())
			...
			require.NoError(err)
			gotHost := map[string]*TargetHostView{}
			for _, s := range gotHosts {
				gotHost[s.PublicId] = s
			}
			...
			foundHosts, err := fetchHosts(context.Background(), rw, projTarget.PublicId)
			require.NoError(err)
			for _, s := range foundHosts {
				assert.NotEmpty(gotHost[s.PublicId])
			}

			target, _, th, err := repo.LookupTarget(context.Background(), projTarget.PublicId)
			require.NoError(err)
			assert.Equal(tt.args.targetVersion+1, target.GetVersion())
			assert.Equal(origTarget.GetVersion(), target.GetVersion()-1)
			assert.Equal(gotHosts, th)
			assert.True(proto.Equal(gotTarget.(*TcpTarget), target.(*TcpTarget)))
		})
	}
}
...
func TestRepository_DeleteTargetHosts(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert, require := assert.New(t), require.New(t)
			hIds := make([]string, 0, tt.args.createCnt)
			if tt.args.createCnt > 0 {
				for i := 0; i < tt.args.createCnt; i++ {
					cats := static.TestCatalogs(t, conn, proj.PublicId, 1)
					hosts := static.TestHosts(t, conn, cats[0].GetPublicId(), 1)
					hIds = append(hIds, hosts[0].PublicId)
				}
			}
			_, _, addedHosts, err := repo.AddTargetHosts(context.Background(), tt.args.target.GetPublicId(), 1, hIds, tt.args.opt...)
			require.NoError(err)
			assert.Equal(tt.args.createCnt, len(addedHosts))

			deleteHosts := make([]string, 0, tt.args.deleteCnt)
			for i := 0; i < tt.args.deleteCnt; i++ {
				deleteHosts = append(deleteHosts, hIds[i])
			}
			var targetId string
			switch {
			case tt.args.targetIdOverride != nil:
			...
			deletedRows, err := repo.DeleteTargetHosts(context.Background(), targetId, targetVersion, deleteHosts, tt.args.opt...)
			if tt.wantErr {
				assert.Error(err)
				assert.Equal(0, deletedRows)
				assert.Truef(errors.Match(errors.T(tt.wantIsErr), err), "unexpected error %s", err.Error())
...
func TestRepository_SetTargetHosts(t *testing.T) {
	t.Parallel()
	conn, _ := db.TestSetup(t, "postgres")
	rw := db.New(conn)
	wrapper := db.TestWrapper(t)
	...
	testCats := static.TestCatalogs(t, conn, proj.PublicId, 1)
	hosts := static.TestHosts(t, conn, testCats[0].GetPublicId(), 5)
	testHostIds := make([]string, 0, len(hosts))
	for _, h := range hosts {
		testHostIds = append(testHostIds, h.PublicId)
	}

	createHostsFn := func() []string {
		results := []string{}
		for i := 0; i < 10; i++ {
			cats := static.TestCatalogs(t, conn, proj.PublicId, 1)
			hosts := static.TestHosts(t, conn, cats[0].GetPublicId(), 1)
			results = append(results, hosts[0].PublicId)
		}
		return results
	}

	setupFn := func(target Target) []*TargetHostView {
		hosts := createHostsFn()
		_, _, created, err := repo.AddTargetHosts(context.Background(), target.GetPublicId(), 1, hosts)
		require.NoError(t, err)
		require.Equal(t, 10, len(created))
		return created
	}
	...
	tests := []struct {
		name             string
		setup            func(Target) []*TargetHostView
		args             args
		wantAffectedRows int
		wantErr          bool
	}{
		...
		},
		{
			...
			args: args{
				target:            TestTcpTarget(t, conn, proj.PublicId, "add-sets"),
				targetVersion:     2, // yep, since setupFn will increment it to 2
				hostIds:           []string{testHostIds[0], testHostIds[1]},
				addToOrigHostSets: true,
			},
			wantErr:          false,
			wantAffectedRows: 2,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert, require := assert.New(t), require.New(t)
			var origHosts []*TargetHostView
			if tt.setup != nil {
				origHosts = tt.setup(tt.args.target)
			}
			if tt.args.addToOrigHostSets {
				origIds := make([]string, 0, len(origHosts))
				for _, s := range origHosts {
					origIds = append(origIds, s.PublicId)
				}
				tt.args.hostIds = append(tt.args.hostIds, origIds...)
			}
			origTarget, _, lookedUpHosts, err := repo.LookupTarget(context.Background(), tt.args.target.GetPublicId())
			require.NoError(err)
			assert.Equal(len(origHosts), len(lookedUpHosts))

			got, affectedRows, err := repo.SetTargetHosts(context.Background(), tt.args.target.GetPublicId(), tt.args.targetVersion, tt.args.hostIds, tt.args.opt...)
			if tt.wantErr {
				require.Error(err)
				t.Log(err)
				return
			...
			require.NoError(err)
			assert.Equal(tt.wantAffectedRows, affectedRows)
			assert.Equal(len(tt.args.hostIds), len(got))

			var wantIds []string
			wantIds = append(wantIds, tt.args.hostIds...)
			sort.Strings(wantIds)
