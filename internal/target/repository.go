...
func (r *Repository) UpdateTarget(ctx context.Context, target Target, version uint32, fieldMaskPaths []string, _ ...Option) (Target, []HostSource, []CredentialSource, int, error) {
	...
	_, err = r.writer.DoTx(
		...
		func(read db.Reader, w db.Writer) error {
			...
			rowsUpdated, err = w.Update(ctx, t, filteredDbMask, filteredNullFields, db.NewOplogMsg(&tOplogMsg), db.WithVersion(&version))
			if err != nil {
				return errors.Wrap(ctx, err, op)
			}
			if rowsUpdated > 1 {
			...
			case updateAddress:
				...
				if err := w.Create(ctx, address, db.NewOplogMsg(&aOplogMsg),
			...
			case deleteAddress:
				...
				rowsUpdated, err = w.Delete(ctx, address, db.NewOplogMsg(&aOplogMsg))
				if err != nil {
