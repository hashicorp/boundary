...
func TestJobRun_New(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr:     true,
			wantErrCode: errors.InvalidParameter,
			wantErrMsg:  "job.NewJobRun: invalid run status: parameter violation: error #100",
		},
		{
			name: "valid",
			args: args{
	...
	t.Run("conflicting-job-run", func(t *testing.T) {
		...
		run, err := NewJobRun(job1.PrivateId, tc.PrivateId)
		require.NoError(err)
		require.NotNil(run)
		...
		run, err = NewJobRun(job2.PrivateId, tc.PrivateId)
		require.NoError(err)
		require.NotNil(run)
		...
		run, err = NewJobRun(job1withCode.PrivateId, tc.PrivateId)
		require.NoError(err)
		require.NotNil(run)
