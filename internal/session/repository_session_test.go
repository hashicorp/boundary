...
func TestRepository_ListSession(t *testing.T) {
	...
	t.Run("withOrder", func(t *testing.T) {
		...
		got, err := repo.ListSessions(context.Background(), WithOrderByCreateTime(true))
		require.NoError(err)
	...
	t.Run("WithSessionIds", func(t *testing.T) {
		...
		got, err := repo.ListSessions(context.Background(), WithSessionIds(withIds...), WithOrderByCreateTime(true))
		require.NoError(err)
