...
func TestRepository_ListCatalogs(t *testing.T) {
	...
	total := 10
	TestCatalogs(t, conn, proj1.GetPublicId(), plg.GetPublicId(), total/2)
	TestCatalogs(t, conn, proj2.GetPublicId(), plg.GetPublicId(), total/2)
	...
	t.Run("no-options", func(t *testing.T) {
		got, plgs, err := repo.ListCatalogs(ctx, []string{proj1.GetPublicId(), proj2.GetPublicId()})
		require.NoError(t, err)
		assert.Equal(t, total, len(got))
		require.Len(t, plgs, 1)
		assert.Equal(t, plgs[0], plg)
	})

	t.Run("withStartPageAfter", func(t *testing.T) {
		assert, require := assert.New(t), require.New(t)

		page1, plgs, err := repo.ListCatalogs(
			context.Background(),
		...
		require.Len(page1, 2)
		require.Len(plgs, 1)
		assert.Equal(plgs[0], plg)
		page2, plgs, err := repo.ListCatalogs(
			context.Background(),
			[]string{proj1.GetPublicId(), proj2.GetPublicId()},
			host.WithLimit(2),
		...
		page3, plgs, err := repo.ListCatalogs(
			context.Background(),
			[]string{proj1.GetPublicId(), proj2.GetPublicId()},
			host.WithLimit(2),
		...
		page4, plgs, err := repo.ListCatalogs(
			context.Background(),
			[]string{proj1.GetPublicId(), proj2.GetPublicId()},
			host.WithLimit(2),
		...
		page5, plgs, err := repo.ListCatalogs(
			context.Background(),
			[]string{proj1.GetPublicId(), proj2.GetPublicId()},
			host.WithLimit(2),
		...
		page6, plgs, err := repo.ListCatalogs(
			context.Background(),
			[]string{proj1.GetPublicId(), proj2.GetPublicId()},
			host.WithLimit(2),
		...
		_, _, _, err = repo.UpdateCatalog(ctx, page1[0].(*HostCatalog), page1[0].GetVersion(), []string{"name"})
		require.NoError(err)
		page7, plgs, err := repo.ListCatalogs(
			context.Background(),
			[]string{proj1.GetPublicId(), proj2.GetPublicId()},
			host.WithLimit(2),
			host.WithStartPageAfterItem(page5[1]),
...
func TestListDeletedCatalogIds(t *testing.T) {
	...
	// Delete a catalog
	c := TestCatalog(t, conn, proj1.GetPublicId(), plg.GetPublicId())
