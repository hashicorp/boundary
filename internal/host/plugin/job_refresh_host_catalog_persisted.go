...
// NextRunIn returns the next run time for the job. It is either the next valid
// refresh time for a secret, or the default value in
// refreshHostCatalogPersistedJobInterval.
func (j *RefreshHostCatalogPersistedJob) NextRunIn() (time.Duration, error) {
	const op = "plugin.(RefreshHostCatalogPersistedJob).NextRunIn"

	// Get the first catalog that has a secret with a non-nil refresh_at_time
	// either equal to the current time or in the future.
	var aggs []*catalogAgg
	if err := j.reader.SearchWhere(
		context.Background(),
		&aggs,
		"persisted_refresh_at_time is not null and persisted_refresh_at_time <= current_timestamp order by persisted_refresh_at_time asc",
		nil,
		db.WithLimit(1),
	); err != nil {
		return refreshHostCatalogPersistedJobInterval, errors.WrapDeprecated(err, op)
	}

	if len(aggs) == 0 {
		// No records need refreshing
		return refreshHostCatalogPersistedJobInterval, nil
	}

	// Assert that RefreshAtTime is non-nil
	if aggs[0].PersistedRefreshAtTime == nil {
		return refreshHostCatalogPersistedJobInterval, errors.NewDeprecated(errors.Internal, op, "PersistedRefreshAtTime is nil")
	}

	// Compute interval from the RefreshAtTime minus the current time
	next := time.Until(aggs[0].PersistedRefreshAtTime.AsTime())

	return next, nil
}

// Name is the unique name of the job.
func (j *RefreshHostCatalogPersistedJob) Name() string {
	return refreshHostCatalogPersistedJobName
}

// Description is the human readable description of the job.
func (j *RefreshHostCatalogPersistedJob) Description() string {
	return "Periodically refreshes secrets for host catalogs."
}

// Status returns the current status of the job. Total is the total number of
// catalogs that are to be refreshed. Completed is the number of sets already
// refreshed.
...
func (j *RefreshHostCatalogPersistedJob) Run(ctx context.Context) error {
	...
	if err := j.reader.SearchWhere(
		ctx,
		&aggs,
		"persisted_refresh_at_time is not null and persisted_refresh_at_time <= current_timestamp",
		nil,
