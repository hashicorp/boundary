...
func (r *Repository) CreateHost(ctx context.Context, scopeId string, h *Host, opt ...Option) (*Host, error) {
	if h == nil {
		return nil, errors.New(errors.InvalidParameter, errors.WithMsg("nil host"))
	}
	if h.Host == nil {
		return nil, errors.New(errors.InvalidParameter, errors.WithMsg("embedded Host"))
	}
	if h.CatalogId == "" {
		return nil, errors.New(errors.InvalidParameter, errors.WithMsg("no catalog id"))
	}
	if h.PublicId != "" {
		return nil, errors.New(errors.InvalidParameter, errors.WithMsg("public id not empty"))
	}
	if scopeId == "" {
		return nil, errors.New(errors.InvalidParameter, errors.WithMsg("no scopeId"))
	}
	h.Address = strings.TrimSpace(h.Address)
	if len(h.Address) < MinHostAddressLength || len(h.Address) > MaxHostAddressLength {
		return nil, errors.New(errors.InvalidParameter, errors.WithMsg("bad address"))
	}
	h = h.clone()
	...
	if opts.withPublicId != "" {
		if !strings.HasPrefix(opts.withPublicId, HostPrefix+"_") {
			return nil, errors.New(errors.InvalidParameter,
		...
		id, err := newHostId()
		if err != nil {
			return nil, errors.New(errors.Unknown, errors.WithMsg("failed to generate hostID"), errors.WithWrap(err))
		}
		h.PublicId = id
	}
	...
	if err != nil {
		return nil, errors.New(errors.Unknown, errors.WithMsg("unable to get oplog wrapper"), errors.WithWrap(err))
	}
	...
	if err != nil {
		...
		case errors.IsUniqueError(err):
			return nil, errors.New(errors.NotUnique,
				errors.WithMsg(fmt.Sprintf("in catalog: %s: name %s already exists", h.CatalogId, h.Name)),
				errors.WithWrap(err))
		case errors.IsCheckConstraintError(err):
			return nil, errors.New(errors.CheckConstraint,
				errors.WithMsg(fmt.Sprintf("in catalog: %s: %q", h.CatalogId, h.Name)),
				errors.WithWrap(err))
		case errors.IsNotNullError(err):
			return nil, errors.New(errors.NotNull,
				errors.WithMsg(fmt.Sprintf("in catalog: %s: %q", h.CatalogId, h.Name)),
				errors.WithWrap(err))
		default:
			return nil, errors.New(errors.Unknown,
				errors.WithMsg(fmt.Sprintf("in catalog: %s: %q", h.CatalogId, h.Name)),
				errors.WithWrap(err))
		}
	}
