...
func (s Service) convertToAuthenticateResponse(ctx context.Context, req *pbs.AuthenticateRequest, authResults *requestauth.VerifyResults, tok *pba.AuthToken) (*pbs.AuthenticateResponse, error) {
	...
	return &pbs.AuthenticateResponse{
		...
		Attrs: &pbs.AuthenticateResponse_AuthTokenResponse{
			AuthTokenResponse: &pb.AuthTokenResponse{
				Id:                      tok.Id,
				ScopeId:                 tok.ScopeId,
				Scope:                   tok.Scope,
				Token:                   tok.Token,
				UserId:                  tok.UserId,
				AuthMethodId:            tok.AuthMethodId,
				AccountId:               tok.AccountId,
				CreatedTime:             tok.CreatedTime,
				UpdatedTime:             tok.UpdatedTime,
				ApproximateLastUsedTime: tok.ApproximateLastUsedTime,
				ExpirationTime:          tok.ExpirationTime,
				TokenType:               req.GetTokenType(),
				AuthorizedActions:       authResults.FetchActionSetForId(ctx, tok.Id, authtokens.IdActions, requestauth.WithResource(res)).Strings(),
			},
		},
	}, nil
}
