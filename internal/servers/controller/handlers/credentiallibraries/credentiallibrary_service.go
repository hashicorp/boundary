...
func toProto(in credential.Library, opt ...handlers.Option) (*pb.CredentialLibrary, error) {
	...
	case vault.Subtype:
		...
		if outputFields.Has(globals.CredentialTypeField) && vaultIn.GetCredentialType() != string(credential.UnspecifiedType) {
			...
			if outputFields.Has(globals.CredentialMappingOverridesField) {
				mapping := vaultIn.GetMapping()
				if mapping != nil {
					m := make(map[string]interface{}, len(mapping))
...
func toStorageVaultLibrary(storeId string, in *pb.CredentialLibrary) (out *vault.CredentialLibrary, err error) {
	...
	case credential.UserPasswordType:
		...
		if username, ok := overrides[string(credential.UsernameField)]; ok {
			mapOpts = append(mapOpts, vault.WithOverrideUsernameAttribute(username.(string)))
		}
		if password, ok := overrides[string(credential.PasswordField)]; ok {
...
func validateMapping(badFields map[string]string, credentialType string, overrides map[string]interface{}) {
	...
	case "":
		...
		return
	case credential.UserPasswordType:
		validFields[string(credential.UsernameField)] = true
		validFields[string(credential.PasswordField)] = true
	default:
	...
	for k, v := range overrides {
		if _, ok := v.(string); !ok {
			badFields[globals.CredentialMappingOverridesField+"."+k] = fmt.Sprintf("Mapping value must be a string, got %T", v)
			continue
		}
