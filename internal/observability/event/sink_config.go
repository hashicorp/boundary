...
type SinkConfig struct {
	...
	TypeConfig       map[string]interface{} `hcl:"type_config"`      // SinkTypeConfig defines type-specific configuration options
	ParsedTypeConfig interface{}            `hcl:"-"`                // ParsedTypeConfig is a pre-caching of the parsed map for known types
}
...
func (sc *SinkConfig) Validate() error {
	...
	switch sc.Type {
	case FileSink:
		if sc.ParsedTypeConfig == nil {
			return fmt.Errorf("%s: missing parsed file sink configuration: %w", op, ErrInvalidParameter)
		}
		fsc, ok := sc.ParsedTypeConfig.(*FileSinkTypeConfig)
		if !ok {
			return fmt.Errorf("%s: invalid parsed file sink configuration: %w", op, ErrInvalidParameter)
		}
		if fsc.FileName == "" {
