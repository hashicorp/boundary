	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, b := range s.bundles {
				addrs = append(addrs, b.ln.BaseListener.Addr().String())
			}
			if tt.assertions != nil {
	...
	tests := []struct {
		...
		},
		{
			...
			assertions: func(t *testing.T, s *Server) {
				...
				require.ErrorIs(t, s.bundles[0].ln.HTTPServer.Serve(s.bundles[0].ln.BaseListener), http.ErrServerClosed)

				// The underlying listener must be closed.
				require.ErrorIs(t, s.bundles[0].ln.BaseListener.Close(), net.ErrClosed)
			},
			expErr: false,
		},
		{
		...
		},
		{
			...
			assertions: func(t *testing.T, s *Server) {
				// The HTTP Server must be closed.
				require.ErrorIs(t, s.bundles[0].ln.HTTPServer.Serve(s.bundles[0].ln.BaseListener), http.ErrServerClosed)
				require.ErrorIs(t, s.bundles[1].ln.HTTPServer.Serve(s.bundles[1].ln.BaseListener), http.ErrServerClosed)

				// The underlying listener must be closed.
				require.ErrorIs(t, s.bundles[0].ln.BaseListener.Close(), net.ErrClosed)
				require.ErrorIs(t, s.bundles[1].ln.BaseListener.Close(), net.ErrClosed)
			},
		},
	}
...
func TestHealthEndpointLifecycle(t *testing.T) {
	...
	rsp, err := http.Get("http://" + tc.Config().Listeners[0].BaseListener.Addr().String() + "/health")
	require.NoError(t, err)
