...
func (r *Repository) AddPrincipalRoles(ctx context.Context, roleId string, roleVersion uint32, principalIds []string, _ ...Option) ([]PrincipalRole, error) {
	...
	for _, id := range userIds {
		...
		if err != nil {
			return nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory user role"))
		}
	...
	for _, id := range groupIds {
		...
		if err != nil {
			return nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory group role"))
		}
	...
	for _, id := range managedGroupIds {
		...
		if err != nil {
			return nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory managed group role"))
		}
		...
		func(reader db.Reader, w db.Writer) error {
			...
			if len(newGrpRoles) > 0 {
				...
				if err := w.CreateItems(ctx, newGrpRoles, db.NewOplogMsgs(&grpOplogMsgs)); err != nil {
					return errors.WrapDeprecated(err, op, errors.WithMsg("unable to add groups"))
				}
			...
			if len(newManagedGrpRoles) > 0 {
				...
				if err := w.CreateItems(ctx, newManagedGrpRoles, db.NewOplogMsgs(&managedGrpOplogMsgs)); err != nil {
					return errors.WrapDeprecated(err, op, errors.WithMsg("unable to add managed groups"))
				}
			...
			if err != nil {
				return errors.WrapDeprecated(err, op, errors.WithMsg("unable to retrieve current principal roles after adds"))
			}
			return nil
		},
	)
...
func (r *Repository) SetPrincipalRoles(ctx context.Context, roleId string, roleVersion uint32, principalIds []string, _ ...Option) ([]PrincipalRole, int, error) {
	...
	_, err = r.writer.DoTx(
		...
		func(reader db.Reader, w db.Writer) error {
			...
			roleTicket, err := w.GetTicket(&role)
			if err != nil {
				return errors.WrapDeprecated(err, op, errors.WithMsg("unable to get ticket for role"))
			}
			updatedRole := allocRole()
			updatedRole.PublicId = roleId
			updatedRole.Version = roleVersion + 1
			...
			if len(toSet.DeleteUserRoles) > 0 ||
				...
				if len(toSet.DeleteUserRoles) > 0 {
					...
					if err != nil {
						return errors.WrapDeprecated(err, op, errors.WithMsg("unable to delete user roles"))
					}
				...
				if len(toSet.DeleteGroupRoles) > 0 {
					...
					if err != nil {
						return errors.WrapDeprecated(err, op, errors.WithMsg("unable to delete groups"))
					}
				...
				if len(toSet.DeleteManagedGroupRoles) > 0 {
					...
					if err != nil {
						return errors.WrapDeprecated(err, op, errors.WithMsg("unable to delete managed groups"))
					}
			...
			currentPrincipals, err = txRepo.ListPrincipalRoles(ctx, roleId)
			if err != nil {
				return errors.WrapDeprecated(err, op, errors.WithMsg("unable to retrieve current principal roles after sets"))
			}
			return nil
		})
	if err != nil {
...
func (r *Repository) DeletePrincipalRoles(ctx context.Context, roleId string, roleVersion uint32, principalIds []string, _ ...Option) (int, error) {
	...
	for _, id := range userIds {
		usrRole, err := NewUserRole(roleId, id)
		if err != nil {
			return db.NoRowsAffected, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory user role"))
		}
	...
	for _, id := range groupIds {
		grpRole, err := NewGroupRole(roleId, id)
		if err != nil {
			return db.NoRowsAffected, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory group role"))
		}
	...
	for _, id := range managedGroupIds {
		managedGrpRole, err := NewManagedGroupRole(roleId, id)
		if err != nil {
			return db.NoRowsAffected, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory managed group role"))
		}
...
func (r *Repository) ListPrincipalRoles(ctx context.Context, roleId string, opt ...Option) ([]PrincipalRole, error) {
	...
	if err := r.list(ctx, &roles, "role_id = ?", []interface{}{roleId}, opt...); err != nil {
		return nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to lookup roles"))
	}
...
func (r *Repository) PrincipalsToSet(ctx context.Context, role *Role, userIds, groupIds, managedGroupIds []string) (*PrincipalSet, error) {
	...
	if err != nil {
		return nil, errors.WrapDeprecated(err, op, errors.WithMsg(fmt.Sprintf("unable to list existing principal role %s", role.PublicId)))
	}
	...
	for _, id := range userIds {
		...
		if _, ok := existingUsers[id]; !ok {
			...
			if err != nil {
				return nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory user role for add"))
			}
			newUserRoles = append(newUserRoles, usrRole)
		}
	}
	...
	for _, id := range groupIds {
		...
		if _, ok := existingGroups[id]; !ok {
			...
			if err != nil {
				return nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory group role for add"))
			}
			newGrpRoles = append(newGrpRoles, grpRole)
		}
	}
	...
	for _, id := range managedGroupIds {
		...
		if _, ok := existingManagedGroups[id]; !ok {
			...
			if err != nil {
				return nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory managed group role for add"))
			}
			newManagedGrpRoles = append(newManagedGrpRoles, managedGrpRole)
		}
	}
	...
	for _, p := range existingUsers {
		if _, ok := userIdsMap[p.PrincipalId]; !ok {
			...
			if err != nil {
				return nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory user role for delete"))
			}
			deleteUserRoles = append(deleteUserRoles, usrRole)
		}
	}
	...
	for _, p := range existingGroups {
		if _, ok := groupIdsMap[p.PrincipalId]; !ok {
			...
			if err != nil {
				return nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory group role for delete"))
			}
			deleteGrpRoles = append(deleteGrpRoles, grpRole)
		}
	}
	...
	for _, p := range existingManagedGroups {
		if _, ok := managedGroupIdsMap[p.PrincipalId]; !ok {
			...
			if err != nil {
				return nil, errors.WrapDeprecated(err, op, errors.WithMsg("unable to create in memory managed group role for delete"))
			}
			deleteManagedGrpRoles = append(deleteManagedGrpRoles, managedGrpRole)
		}
	}
