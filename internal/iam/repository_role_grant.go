...
// grantsForUserProjectResourcesGlobalScope returns the grants for the user for resources that can
// only be project scoped.
func (r *Repository) grantsForUserProjectResourcesGlobalScope(
	ctx context.Context,
	userId,
	reqScopeId string,
	res resource.Type,
) (perms.GrantTuples, error) {
	const op = "iam.(Repository).grantsForUserProjectResourcesGlobalScope"
	if userId == "" {
		return nil, errors.New(ctx, errors.InvalidParameter, op, "missing user id")
	}
	if reqScopeId == "" {
