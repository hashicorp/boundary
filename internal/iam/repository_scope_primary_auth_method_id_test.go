...
func Test_Repository_UpdateScope_AccountInfoAuthMethodId(t *testing.T) {
	...
	t.Run("update-AccountInfoAuthMethodId", func(t *testing.T) {
		...
		am := oidc.TestAuthMethod(t, conn, databaseWrapper, org.PublicId, oidc.ActivePublicState, "alice-rp", "fido", oidc.WithSigningAlgs(oidc.RS256), oidc.WithApiUrl(oidc.TestConvertToUrls(t, "https://www.alice.com/callback")...))
