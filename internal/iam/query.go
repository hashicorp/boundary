...
const (
    ...
    `
	grantsForUserProjectResourcesQuery = `
	with
    users (id) as (
      select public_id
        from iam_user
    ...
    org_roles as (
      select iam_role_org.public_id             as role_id,
             ...
             iam_role_org.grant_this_role_scope as grant_this_role_scope,
             coalesce(individual.scope_id, '')  as individual_grant_scope,
             roles_withs_grants.canonical_grant  as canonical_grant
        from iam_role_org
        join roles_with_grants
          on roles_with_grants.role_id = iam_role_org.public_id
        left join iam_role_org_individual_project_grant_scope individual
          on individual.role_id = iam_role_org.public_id
       where iam_role_org.grant_scope = 'children'
          or (
              iam_role_org.grant_scope = 'individual' and
              individual.scope_id = @request_scope
          )
    ),
    project_roles as (
