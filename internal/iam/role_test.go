...
// TODO add tests for individual role type
...
//				rsc, err := r.toResource()
//				require.NoError(err)
//				_, err = rw.Update(context.Background(), rsc, tt.args.fieldMaskPaths, tt.args.nullPaths)
//				require.NoError(err)
//			}
//
//			id := testId(t)
...
//				assert.Contains(err.Error(), tt.wantErrMsg)
//				err = db.TestVerifyOplog(t, rw, secondRoleId, db.WithOperation(oplog.OpType_OP_TYPE_UPDATE), db.WithCreateNotBefore(10*time.Second))
//				require.Error(err)
...
//			foundRole.PublicId = secondRoleId
...
//		rsc, err := projRole.toResource()
//		require.NoError(err)
//		updatedRows, err := rw.Update(context.Background(), rsc, []string{"Name"}, nil)
//		require.NoError(err)
...
func TestRole_Actions(t *testing.T) {
	...
	assert.Equal(globalActions[action.Create.String()], action.Create)
	assert.Equal(globalActions[action.Update.String()], action.Update)
	assert.Equal(globalActions[action.Read.String()], action.Read)
	assert.Equal(globalActions[action.Delete.String()], action.Delete)
	assert.Equal(globalActions[action.AddGrants.String()], action.AddGrants)
	assert.Equal(globalActions[action.RemoveGrants.String()], action.RemoveGrants)
	assert.Equal(globalActions[action.SetGrants.String()], action.SetGrants)
	assert.Equal(globalActions[action.AddPrincipals.String()], action.AddPrincipals)
	assert.Equal(globalActions[action.RemovePrincipals.String()], action.RemovePrincipals)
	assert.Equal(globalActions[action.SetPrincipals.String()], action.SetPrincipals)

	org := &orgRole{}
	orgActions := org.Actions()
	assert.Equal(orgActions[action.Create.String()], action.Create)
	assert.Equal(orgActions[action.Update.String()], action.Update)
	assert.Equal(orgActions[action.Read.String()], action.Read)
	assert.Equal(orgActions[action.Delete.String()], action.Delete)
	assert.Equal(orgActions[action.AddGrants.String()], action.AddGrants)
	assert.Equal(orgActions[action.RemoveGrants.String()], action.RemoveGrants)
	assert.Equal(orgActions[action.SetGrants.String()], action.SetGrants)
	assert.Equal(orgActions[action.AddPrincipals.String()], action.AddPrincipals)
	assert.Equal(orgActions[action.RemovePrincipals.String()], action.RemovePrincipals)
	assert.Equal(orgActions[action.SetPrincipals.String()], action.SetPrincipals)

	proj := &projectRole{}
	projActions := proj.Actions()
	assert.Equal(projActions[action.Create.String()], action.Create)
	assert.Equal(projActions[action.Update.String()], action.Update)
	assert.Equal(projActions[action.Read.String()], action.Read)
	assert.Equal(projActions[action.Delete.String()], action.Delete)
	assert.Equal(projActions[action.AddGrants.String()], action.AddGrants)
	assert.Equal(projActions[action.RemoveGrants.String()], action.RemoveGrants)
	assert.Equal(projActions[action.SetGrants.String()], action.SetGrants)
	assert.Equal(projActions[action.AddPrincipals.String()], action.AddPrincipals)
	assert.Equal(projActions[action.RemovePrincipals.String()], action.RemovePrincipals)
	assert.Equal(projActions[action.SetPrincipals.String()], action.SetPrincipals)
}
...
func TestRole_SetTableName(t *testing.T) {
	t.Run("baseRole", func(t *testing.T) {
	...
	t.Run("orgRole", func(t *testing.T) {
	...
	t.Run("projectRole", func(t *testing.T) {
