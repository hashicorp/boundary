...
func Test_TimestampValue(t *testing.T) {
	...
	t.Run("invalid ts", func(t *testing.T) {
		...
		assert.Equal(t, err.Error(), "timestamp: seconds:253402300800 after 10000-01-01")
...
func Test_TimestampScan(t *testing.T) {
	...
	t.Run("valid", func(t *testing.T) {
		...
		err := ts.Scan(v)
		assert.NilError(t, err)
		assert.Check(t, reflect.DeepEqual(ts.Timestamp, &timestamp.Timestamp{Seconds: 0, Nanos: 0}))
	})
	t.Run("valid default time", func(t *testing.T) {
		...
		err := ts.Scan(v)
		assert.NilError(t, err)
		assert.Check(t, reflect.DeepEqual(ts.Timestamp, &timestamp.Timestamp{Seconds: -62135596800, Nanos: 0}))
	})
	t.Run("invalid type", func(t *testing.T) {
		...
		err := ts.Scan(v)
		assert.Check(t, err != nil)
		assert.Equal(t, err.Error(), "Not a protobuf Timestamp")
	})
	t.Run("invalid time", func(t *testing.T) {
		...
		err := ts.Scan(v)
		assert.Check(t, err != nil)
		assert.Equal(t, err.Error(), "error converting the timestamp: timestamp: seconds:253402300800 after 10000-01-01")
	})
}
