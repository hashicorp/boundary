...
func TestRepository_ListUsers(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert, require := assert.New(t), require.New(t)
			db.TestDeleteWhere(t, conn, iam.AllocUser(), "public_id != 'u_anon' and public_id != 'u_auth' and public_id != 'u_recovery'")
			testUsers := []*iam.User{}
...
func TestRepository_AssociateAccounts(t *testing.T) {
	...
	createAccountsFn := func(prefix string) []string {
		db.TestDeleteWhere(t, conn, allocAuthAccount(), "iam_user_id = ?", user.PublicId)
		results := []string{}
		for i := 0; i < 5; i++ {
