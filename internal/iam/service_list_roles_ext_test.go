...
func TestService_ListRoles(t *testing.T) {
	...
	o := iam.TestOrg(t, repo, iam.WithSkipDefaultRoleCreation(true))
	...
	roles := []*iam.Role{}
	for i := 0; i < 5; i++ {
		roles = append(roles, iam.TestRole(t, conn, o.GetPublicId()))
	}

	at := authtoken.TestAuthToken(t, conn, kms, o.GetPublicId())

	// Run analyze to update role estimate
	...
	t.Run("List validation", func(t *testing.T) {
		...
		t.Run("missing grants hash", func(t *testing.T) {
			...
			_, err := iam.ListRoles(ctx, nil, 1, filterFunc, repo, []string{"scopeId"})
		...
		t.Run("zero page size", func(t *testing.T) {
			...
			_, err := iam.ListRoles(ctx, []byte("some hash"), 0, filterFunc, repo, []string{"scopeId"})
		...
		t.Run("negative page size", func(t *testing.T) {
			...
			_, err := iam.ListRoles(ctx, []byte("some hash"), -1, filterFunc, repo, []string{"scopeId"})
			require.ErrorContains(t, err, "page size must be at least 1")
		})
		t.Run("nil filter func", func(t *testing.T) {
			t.Parallel()
			_, err := iam.ListRoles(ctx, []byte("some hash"), 1, nil, repo, []string{"scopeId"})
		...
		t.Run("nil repo", func(t *testing.T) {
			...
			_, err := iam.ListRoles(ctx, []byte("some hash"), 1, filterFunc, nil, []string{"scopeId"})
	...
	t.Run("ListRefresh validation", func(t *testing.T) {
		...
		t.Run("missing grants hash", func(t *testing.T) {
			...
			tok, err := refreshtoken.New(ctx, time.Now(), time.Now(), resource.Role, []byte("some hash"), "some-id", time.Now())
			require.NoError(t, err)
			_, err = iam.ListRolesRefresh(ctx, nil, 1, filterFunc, tok, repo, []string{"scopeId"})
			require.ErrorContains(t, err, "missing grants hash")
		})
		t.Run("zero page size", func(t *testing.T) {
			t.Parallel()
			...
			tok, err := refreshtoken.New(ctx, time.Now(), time.Now(), resource.Role, []byte("some hash"), "some-id", time.Now())
			require.NoError(t, err)
			_, err = iam.ListRolesRefresh(ctx, []byte("some hash"), 0, filterFunc, tok, repo, []string{"scopeId"})
			require.ErrorContains(t, err, "page size must be at least 1")
		})
		t.Run("negative page size", func(t *testing.T) {
			t.Parallel()
			...
			tok, err := refreshtoken.New(ctx, time.Now(), time.Now(), resource.Role, []byte("some hash"), "some-id", time.Now())
			require.NoError(t, err)
			_, err = iam.ListRolesRefresh(ctx, []byte("some hash"), -1, filterFunc, tok, repo, []string{"scopeId"})
			require.ErrorContains(t, err, "page size must be at least 1")
		})
		t.Run("nil filter func", func(t *testing.T) {
			t.Parallel()
			tok, err := refreshtoken.New(ctx, time.Now(), time.Now(), resource.Role, []byte("some hash"), "some-id", time.Now())
			require.NoError(t, err)
			_, err = iam.ListRolesRefresh(ctx, []byte("some hash"), 1, nil, tok, repo, []string{"scopeId"})
			require.ErrorContains(t, err, "missing filter item callback")
		})
		t.Run("nil token", func(t *testing.T) {
			...
			_, err = iam.ListRolesRefresh(ctx, []byte("some hash"), 1, filterFunc, nil, repo, []string{"scopeId"})
		...
		t.Run("nil repo", func(t *testing.T) {
			...
			tok, err := refreshtoken.New(ctx, time.Now(), time.Now(), resource.Role, []byte("some hash"), "some-id", time.Now())
			require.NoError(t, err)
			_, err = iam.ListRolesRefresh(ctx, []byte("some hash"), 1, filterFunc, tok, nil, []string{"scopeId"})
			require.ErrorContains(t, err, "missing repo")
		})
		t.Run("nil scope ids", func(t *testing.T) {
			t.Parallel()
	...
	t.Run("simple pagination", func(t *testing.T) {
		...
		resp, err := iam.ListRoles(ctx, []byte("some hash"), 1, filterFunc, repo, []string{o.GetPublicId()})
		require.NoError(t, err)
		...
		require.Empty(t,
			cmp.Diff(
				resp.Items[0],
				roles[0],
				cmpopts.IgnoreUnexported(iam.Role{}, iamstore.Role{}, timestamp.Timestamp{}, timestamppb.Timestamp{}),
			),
		)

		resp2, err := iam.ListRolesRefresh(ctx, []byte("some hash"), 1, filterFunc, resp.RefreshToken, repo, []string{o.GetPublicId()})
		require.NoError(t, err)
		...
		resp3, err := iam.ListRolesRefresh(ctx, []byte("some hash"), 1, filterFunc, resp2.RefreshToken, repo, []string{o.GetPublicId()})
		require.NoError(t, err)
		...
		resp4, err := iam.ListRolesRefresh(ctx, []byte("some hash"), 1, filterFunc, resp3.RefreshToken, repo, []string{o.GetPublicId()})
		require.NoError(t, err)
		...
		resp5, err := iam.ListRolesRefresh(ctx, []byte("some hash"), 1, filterFunc, resp4.RefreshToken, repo, []string{o.GetPublicId()})
		require.NoError(t, err)
		...
		resp6, err := iam.ListRolesRefresh(ctx, []byte("some hash"), 1, filterFunc, resp5.RefreshToken, repo, []string{o.GetPublicId()})
		require.NoError(t, err)
	...
	t.Run("simple pagination with aggressive filtering", func(t *testing.T) {
		filterFunc := func(_ context.Context, r *iam.Role) (bool, error) {
			return r.GetPublicId() == roles[len(roles)-1].GetPublicId(), nil
		}
		resp, err := iam.ListRoles(ctx, []byte("some hash"), 1, filterFunc, repo, []string{o.GetPublicId()})
		require.NoError(t, err)
		require.NotNil(t, resp.RefreshToken)
	...
	t.Run("simple pagination with deletion", func(t *testing.T) {
		...
		deletedRoleId := roles[0].GetPublicId()
