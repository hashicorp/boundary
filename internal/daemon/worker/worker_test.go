...
func TestSetupWorkerAuthStorage(t *testing.T) {
	...
	pAssert, pRequire := assert.New(t), require.New(t)
	ts := db.TestWrapper(t)
	keyId, err := ts.KeyId(ctx)
	pRequire.NoError(err)
	...
	pRequire.NoError(err)
	pAssert.Equal(keyId, wKeyId)

	err = tw.Worker().Shutdown()
	pRequire.NoError(err)

	// Now run tests to ensure that we are handling storage properly
	tests := []struct {
		name       string
		in         *Config
		expErr     bool
