...
func (s Service) ListCredentialStores(ctx context.Context, req *pbs.ListCredentialStoresRequest) (*pbs.ListCredentialStoresResponse, error) {
	...
	if req.GetListToken() == "" {
		...
		case *listtoken.PaginationToken:
			sortBy = "created_time"
			listResp, err = service.ListPage(ctx, grantsHash, pageSize, filterItemFn, listToken, scopeIds)
			if err != nil {
				return nil, err
			}
		case *listtoken.StartRefreshToken:
			sortBy = "updated_time"
			listResp, err = service.ListRefresh(ctx, grantsHash, pageSize, filterItemFn, listToken, scopeIds)
			if err != nil {
				return nil, err
			}
		case *listtoken.RefreshToken:
			sortBy = "updated_time"
			listResp, err = service.ListRefreshPage(ctx, grantsHash, pageSize, filterItemFn, listToken, scopeIds)
			if err != nil {
				return nil, err
			}
		default:
