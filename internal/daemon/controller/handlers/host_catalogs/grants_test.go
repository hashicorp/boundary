		...
		for _, tc := range testcases {
			t.Run(tc.name, func(t *testing.T) {
				...
				for input, wantErr := range tc.inputWantErrMap {
					...
					require.NoError(t, err)
					handlers.AssertOutputFields(t, got.Item, tc.expectOutfields)
				}
			})
		}
	})
...
func TestGrants_WriteActions(t *testing.T) {
	...
	t.Run("update", func(t *testing.T) {
		...
		testcases := []struct {
			name                        string
			setupScopesResourcesAndUser func(t *testing.T, conn *db.DB, iamRepo *iam.Repository, kmsCache *kms.Kms) (*static.HostCatalog, func() (*iam.User, string))
			wantErr                     error
			expectOutfields             []string
		}{
			{
				...
				setupScopesResourcesAndUser: func(t *testing.T, conn *db.DB, iamRepo *iam.Repository, kmsCache *kms.Kms) (*static.HostCatalog, func() (*iam.User, string)) {
					...
					return hc, iam.TestUserDirectGrantsFunc(t, conn, kmsCache, globals.GlobalPrefix, password.TestAccountFunc(t, conn), []iam.TestRoleGrantsRequest{
						{
							RoleScopeID: proj.PublicId,
	...
	t.Run("delete", func(t *testing.T) {
		...
		testcases := []struct {
			name                        string
			setupScopesResourcesAndUser func(t *testing.T, conn *db.DB, iamRepo *iam.Repository, kmsCache *kms.Kms) (*static.HostCatalog, func() (user *iam.User, accountId string))
			wantErr                     error
		}{
			{
