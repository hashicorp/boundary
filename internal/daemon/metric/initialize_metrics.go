...
// combinations.
func InitializeHttpCollectors(r prometheus.Registerer, v prometheus.ObserverVec, labels LabelNames) {
	if r == nil {
		return
	}
	r.MustRegister(v)
	...
	for _, sc := range expectedHttpErrCodes {
		v.With(labels.ToPromLabels(strconv.Itoa(sc), p, strings.ToLower(method)))
	}
