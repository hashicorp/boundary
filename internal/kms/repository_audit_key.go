...
// audit key version. There are no valid options at this time.
func (r *Repository) CreateAuditKey(ctx context.Context, rkvWrapper wrapping.Wrapper, key []byte, _ ...Option) (*AuditKey, *AuditKeyVersion, error) {
	const op = "kms.(Repository).CreateAuditKey"
	...
	_, err := r.writer.DoTx(
		...
		func(reader db.Reader, w db.Writer) error {
			var err error
			if returnedDk, returnedDv, err = createAuditKeyTx(ctx, reader, w, rkvWrapper, key); err != nil {
	...
	return returnedDk.(*AuditKey), returnedDv.(*AuditKeyVersion), nil
}

// createAuditKeyTx inserts into the db (via db.Writer) and returns the new audit key
// and audit key version. This function encapsulates all the work required within
// a db.TxHandler and allows this capability to be shared with the iam repo.
func createAuditKeyTx(ctx context.Context, r db.Reader, w db.Writer, rkvWrapper wrapping.Wrapper, key []byte) (*AuditKey, *AuditKeyVersion, error) {
	const op = "kms.createAuditKeyTx"
	if rkvWrapper == nil {
...
// LookupAuditKey will look up a key in the repository.  If the key is not
// found, it will return nil, nil.
func (r *Repository) LookupAuditKey(ctx context.Context, privateId string, _ ...Option) (*AuditKey, error) {
	const op = "kms.(Repository).LookupAuditKey"
	if privateId == "" {
...
// DeleteAuditKey deletes the key for the provided id from the
// repository returning a count of the number of records deleted.  All options
// are ignored.
func (r *Repository) DeleteAuditKey(ctx context.Context, privateId string, _ ...Option) (int, error) {
	const op = "kms.(Repository).DeleteAuditKey"
	if privateId == "" {
...
// ListAuditKeys will list the keys.  Supports the WithLimit option.
func (r *Repository) ListAuditKeys(ctx context.Context, opt ...Option) ([]Dek, error) {
	const op = "kms.(Repository).ListAuditKeys"
	var keys []*AuditKey
