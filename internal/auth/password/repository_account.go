...
// GetTotalAccounts returns an estimate of the number of password accounts
func (r *Repository) GetTotalAccounts(ctx context.Context) (int, error) {
	const op = "password.(Repository).GetTotalAccounts"
	rows, err := r.reader.Query(ctx, estimateCountPasswordAccounts, nil)
	if err != nil {
		return 0, errors.Wrap(ctx, err, op, errors.WithMsg("failed to query total password accounts"))
	}
	var count int
	for rows.Next() {
...
// ListDeletedAccountIds lists the public IDs of any password accounts deleted since the timestamp provided.
// Supported options:
