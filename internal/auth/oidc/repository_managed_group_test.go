...
func TestRepository_ListManagedGroups_Pagination(t *testing.T) {
	...
	testRepo, err := NewRepository(ctx, rw, rw, kmsCache)
	assert.NoError(t, err)
	require.NotNil(t, testRepo)

	page1, err := testRepo.ListManagedGroups(ctx, authMethod1.GetPublicId(), WithLimit(2))
	require.NoError(t, err)
	require.Len(t, page1, 2)
	page2, err := testRepo.ListManagedGroups(ctx, authMethod1.GetPublicId(), WithLimit(2), WithStartPageAfterItem(page1[1]))
	require.NoError(t, err)
	...
	page3, err := testRepo.ListManagedGroups(ctx, authMethod1.GetPublicId(), WithLimit(2), WithStartPageAfterItem(page2[1]))
	require.NoError(t, err)
	require.Len(t, page3, 1)
	for _, item := range append(page1, page2...) {
		assert.NotEqual(t, item.GetPublicId(), page3[0].GetPublicId())
	}
	page4, err := testRepo.ListManagedGroups(ctx, authMethod1.GetPublicId(), WithLimit(2), WithStartPageAfterItem(page3[0]))
	require.NoError(t, err)
...
func Test_listDeletedIds(t *testing.T) {
	...
	authMethod := TestAuthMethod(
		t, testConn, orgDbWrapper, org.PublicId, ActivePrivateState,
		"alice-rp", "fido",
		WithSigningAlgs(RS256),
		WithIssuer(TestConvertToUrls(t, "https://www.alice.com")[0]),
		WithApiUrl(TestConvertToUrls(t, "https://www.alice.com/callback")[0]),
	)
	mg := TestManagedGroup(t, testConn, authMethod, TestFakeManagedGroupFilter)
	require.NoError(t, err)
