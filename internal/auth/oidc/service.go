...
func encryptMessage(ctx context.Context, wrapper wrapping.Wrapper, am *AuthMethod, m proto.Message) (encodedEncryptedState string, e error) {
	...
	if am.ScopeId == "" {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, "missing auth method scope id")
	}
	if am.PublicId == "" {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, "missing auth method public id")
	}
	if m == nil {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, "missing message to encrypt")
	}
	switch v := m.(type) {
	case *request.State, *request.Token:
	default:
		return "", errors.NewDeprecated(errors.InvalidParameter, op, fmt.Sprintf("unsupported message type %v for encryption", v))
	}
	...
	if err != nil {
		return "", errors.NewDeprecated(errors.Encrypt, op, "unable to encrypt message", errors.WithWrap(err))
	}
	...
	if err != nil {
		return "", errors.NewDeprecated(errors.Unknown, op, "unable to marshal encrypted message", errors.WithWrap(err))
	}
...
func decryptMessage(ctx context.Context, wrappingWrapper wrapping.Wrapper, wrappedRequest *request.Wrapper) (messageBytes []byte, e error) {
	...
	if wrappedRequest == nil {
		return nil, errors.NewDeprecated(errors.InvalidParameter, op, "missing wrapped request")
	}
	if wrappingWrapper == nil {
		return nil, errors.NewDeprecated(errors.InvalidParameter, op, "missing wrapping wrapper")
	}
	var blobInfo wrapping.EncryptedBlobInfo
	if err := proto.Unmarshal(wrappedRequest.Ct, &blobInfo); err != nil {
		return nil, errors.NewDeprecated(errors.Unknown, op, "unable to marshal blob info", errors.WithWrap(err))
	}
	...
	if err != nil {
		return nil, errors.NewDeprecated(errors.Decrypt, op, "unable to decrypt message", errors.WithWrap(err))
	}
...
func UnwrapMessage(ctx context.Context, encodedWrappedMsg string) (*request.Wrapper, error) {
	...
	if err != nil {
		return nil, errors.NewDeprecated(errors.Unknown, op, "unable to decode message", errors.WithWrap(err))
	}
	var wrapper request.Wrapper
	if err := proto.Unmarshal(decoded, &wrapper); err != nil {
		return nil, errors.NewDeprecated(errors.Unknown, op, "unable to marshal encoded/encrypted message", errors.WithWrap(err))
	}
	return &wrapper, nil
}
...
func requestWrappingWrapper(ctx context.Context, k *kms.Kms, scopeId, authMethodId string, opt ...Option) (wrapping.Wrapper, error) {
	...
	if err != nil {
		return nil, errors.NewDeprecated(errors.Encrypt, op, "unable to generate key", errors.WithWrap(err))
	}
