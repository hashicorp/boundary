...
func Test_requestWrappingWrapper(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(err)
			wantKeyVersionId := derivedKeyVersionId(derivedKeyPurposeState, keyVersionId, authMethodId)
			kmsCache.GetDerivedPurposeCache().Delete(wantKeyVersionId)

			reqWrapper, err := requestWrappingWrapper(ctx, repo.kms, scopeId, authMethodId, tt.opt...)
			if tt.wantErrMatch != nil {
				require.Error(err)
				assert.Empty(reqWrapper)
				cachedWrapper, found := kmsCache.GetDerivedPurposeCache().Load(wantKeyVersionId)
				assert.False(found)
			...
			keyVersionId, err = reqWrapper.KeyId(ctx)
			require.NoError(err)
			wrapperType, err := reqWrapper.Type(ctx)
			require.NoError(err)
			assert.Equalf(wantKeyVersionId, keyVersionId, "expected key id %s and got: %s", wantKeyVersionId, keyVersionId)
