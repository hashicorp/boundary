...
func Callback(
	...
	if oidcRepoFn == nil {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, "missing oidc repository function")
	}
	if iamRepoFn == nil {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, "missing iam repository function")
	}
	if atRepoFn == nil {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, "missing auth token repository function")
	}
	if am == nil {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, "missing auth method")
	}
	if state == "" {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, "missing state")
	}
	if code == "" {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, "missing code")
	}
	...
	if stateWrapper.AuthMethodId != am.GetPublicId() {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, fmt.Sprintf("%s auth method id does not match request wrapper auth method id: %s", am.GetPublicId(), stateWrapper))
	}
	...
	if err := proto.Unmarshal(stateBytes, &reqState); err != nil {
		return "", errors.NewDeprecated(errors.Unknown, op, "unable to unmarshal request state", errors.WithWrap(err))
	}
	...
	if err != nil {
		return "", errors.NewDeprecated(errors.Unknown, op, "unable to get provider config hash", errors.WithWrap(err))
	}
	if reqState.ProviderConfigHash != hash && am.OperationalState == string(InactiveState) {
		return "", errors.NewDeprecated(errors.AuthMethodInactive, op, "auth method configuration changed during in-flight authentication attempt")
	}

	// before proceeding, make sure the request hasn't timed out
	if time.Now().After(reqState.ExpirationTime.Timestamp.AsTime()) {
		return "", errors.NewDeprecated(errors.AuthAttemptExpired, op, "request state has expired")
	}
	...
	if strings.TrimSpace(am.ApiUrl) == "" {
		return "", errors.NewDeprecated(errors.InvalidParameter, op, "empty api URL")
	}
	oidcRequest, err := oidc.NewRequest(AttemptExpiration, fmt.Sprintf(CallbackEndpoint, am.ApiUrl), opts...)
	if err != nil {
		return "", errors.NewDeprecated(errors.Unknown, op, "unable to create oidc request for token exchange", errors.WithWrap(err))
	}
	tk, err := provider.Exchange(ctx, oidcRequest, state, code)
	if err != nil {
		return "", errors.NewDeprecated(errors.Unknown, op, "unable to complete exchange with oidc provider", errors.WithWrap(err))
	}
	...
	if err := tk.IDToken().Claims(&idTkClaims); err != nil {
		return "", errors.NewDeprecated(errors.Unknown, op, "unable to parse ID Token claims", errors.WithWrap(err))
	}
	...
	if userInfoTokenSource != nil {
		...
		if !ok {
			return "", errors.NewDeprecated(errors.Unknown, op, "subject is not present in ID Token, which should not be possible")
		}
		if err := provider.UserInfo(ctx, userInfoTokenSource, sub, &userInfoClaims); err != nil {
			return "", errors.NewDeprecated(errors.Unknown, op, "unable to get user info from provider", errors.WithWrap(err))
		}
	}
