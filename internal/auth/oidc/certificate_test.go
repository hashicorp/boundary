...
func TestCertificate_Delete(t *testing.T) {
	...
	cert, pem := testGenerateCA(t, "localhost")
	...
	testResource := func(authMethodId string, Certificate string) *Certificate {
		c, err := NewCertificate(authMethodId, pem)
		require.NoError(t, err)
		return c
	}
	tests := []struct {
		...
		{
			name:            "valid",
			Certificate:     testResource(testAuthMethod.PublicId, "https://alice.com/callback"),
			wantErr:         false,
			wantRowsDeleted: 1,
		},
		{
			name:            "bad-OidcMethodId",
			Certificate:     testResource(testAuthMethod.PublicId, "https://bad-id/callback"),
		...
		{
			name:            "bad-Url",
			Certificate:     testResource(testAuthMethod.PublicId, "https://bad-url.com/callback"),
