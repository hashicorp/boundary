...
func (r *Repository) estimatedManagedGroupCount(ctx context.Context) (uint, error) {
	...
	if err != nil {
		return 0, errors.Wrap(ctx, err, op, errors.WithMsg("failed to query estimate oidc managed group counts"))
	}
	...
	if err != nil {
		return 0, errors.Wrap(ctx, err, op, errors.WithMsg("failed to query total oidc managed group counts"))
	}
	var count uint
	for rows.Next() {
		if err := r.reader.ScanRows(ctx, rows, &count); err != nil {
			return 0, errors.Wrap(ctx, err, op, errors.WithMsg("failed to scan total oidc managed group counts"))
		}
	}
	return count, nil
}
