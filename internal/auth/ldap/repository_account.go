...
func (r *Repository) estimatedAccountCount(ctx context.Context) (uint, error) {
	...
	if err != nil {
		return 0, errors.Wrap(ctx, err, op, errors.WithMsg("failed to query estimate ldap account count"))
	}
	var estimatedCount int
	for rows.Next() {
		if err := r.reader.ScanRows(ctx, rows, &estimatedCount); err != nil {
			return 0, errors.Wrap(ctx, err, op, errors.WithMsg("failed to scan estimate ldap account count"))
		}
	}
	if estimatedCount >= 0 {
	...
	if err != nil {
		return 0, errors.Wrap(ctx, err, op, errors.WithMsg("failed to query total ldap account count"))
	}
	var count uint
	for rows.Next() {
		if err := r.reader.ScanRows(ctx, rows, &count); err != nil {
			return 0, errors.Wrap(ctx, err, op, errors.WithMsg("failed to scan total ldap account count"))
		}
	}
	return count, nil
}
