...
func TestRepository_lookupUpUser(t *testing.T) {
	...
	t.Run("soft-deleted", func(t *testing.T) {
		...
		tu, err := testLookupUser(t, s, "user", at2.UserId)
		assert.NoError(t, err)
		...
		tu, err = testLookupUser(t, s, "user", tu.Id)
		assert.NoError(t, err)
	...
	t.Run("hard-deleted", func(t *testing.T) {
		...
		// should be found in underlying user table as well
		tu, err := testLookupUser(t, s, "user", at3.UserId)
		assert.NoError(t, err)
		...
		_, err = testLookupUser(t, s, "user", tu.Id)
		assert.Error(t, err)
...
func testLookupUser(t *testing.T, conn any, tableName, id string) (*testUser, error) {
	t.Helper()
	var rw db.Reader
	switch v := conn.(type) {
	case *db.DB:
	...
	err := rw.LookupById(context.Background(), u, db.WithTable(tableName))
	switch {
	case err == nil:
		return u, nil
	default:
...
func TestUpsertUserAndAuthToken(t *testing.T) {
	...
	t.Run("hard-and-soft-delete-oldest-user", func(t *testing.T) {
		...
		for i := 1; i <= usersLimit; i++ {
			...
			require.NoError(t, err)
			_, err = testLookupUser(t, s, "user", foundUser.Id)
			assert.NoError(t, err)
		}

		{
			...
			foundTestUser, err := testLookupUser(t, s, "user", "u_1")
			assert.Error(t, err)
		...
		}
		{
			...
			foundTestUser, err := testLookupUser(t, s, "user", "u_2")
			assert.NoError(t, err)
