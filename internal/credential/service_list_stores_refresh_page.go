...
// ListRefreshPage lists up to page size credential stores, filtering out entries that
...
// last response.
func (s *StoreService) ListRefreshPage(
	ctx context.Context,
	grantsHash []byte,
	pageSize int,
	...
	const op = "credential.(*StoreService).ListRefreshPage"
	...
	listItemsFn := func(ctx context.Context, lastPageItem Store, limit int) ([]Store, time.Time, error) {
		...
		if lastPageItem != nil {
			return s.listRefresh(ctx, projectIds, since, lastPageItem, limit)
		}
		lastItem, err := tok.LastItem(ctx)
		if err != nil {
			return nil, time.Time{}, err
		}
		return s.listRefresh(ctx, projectIds, since, lastItem, limit)
	}
	...
	return pagination.ListRefreshPage(ctx, grantsHash, pageSize, filterItemFn, listItemsFn, s.estimatedCount, listDeletedIDsFn, tok)
}
