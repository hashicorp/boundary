...
// ListPage lists up to page size credential stores, filtering out entries that
...
func (s *StoreService) ListPage(
	...
	const op = "credential.(*StoreService).ListPage"
	...
	listItemsFn := func(ctx context.Context, lastPageItem Store, limit int) ([]Store, time.Time, error) {
		...
		return s.list(ctx, projectIds, lastItem, limit)
	}

	return pagination.ListPage(ctx, grantsHash, pageSize, filterItemFn, listItemsFn, s.estimatedCount, tok)
}
