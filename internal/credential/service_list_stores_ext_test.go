...
func TestStoreService_List(t *testing.T) {
	...
	t.Run("ListLibraries validation", func(t *testing.T) {
		...
		t.Run("missing grants hash", func(t *testing.T) {
			...
			_, err := service.List(ctx, nil, 1, filterFunc, []string{prj.GetPublicId()})
		...
		t.Run("zero page size", func(t *testing.T) {
			...
			_, err := service.List(ctx, []byte("some hash"), 0, filterFunc, []string{prj.GetPublicId()})
		...
		t.Run("negative page size", func(t *testing.T) {
			...
			_, err := service.List(ctx, []byte("some hash"), -1, filterFunc, []string{prj.GetPublicId()})
			require.ErrorContains(t, err, "page size must be at least 1")
		})
		t.Run("nil filter func", func(t *testing.T) {
			t.Parallel()
			_, err := service.List(ctx, []byte("some hash"), 1, nil, []string{prj.GetPublicId()})
	...
	t.Run("ListLibrariesRefresh validation", func(t *testing.T) {
		...
		t.Run("missing grants hash", func(t *testing.T) {
			...
			tok, err := refreshtoken.New(ctx, time.Now(), time.Now(), resource.CredentialStore, []byte("some hash"), "some-id", time.Now())
			require.NoError(t, err)
			_, err = service.ListRefresh(ctx, nil, 1, filterFunc, tok, []string{prj.GetPublicId()})
			require.ErrorContains(t, err, "missing grants hash")
		})
		t.Run("zero page size", func(t *testing.T) {
			t.Parallel()
			...
			tok, err := refreshtoken.New(ctx, time.Now(), time.Now(), resource.CredentialStore, []byte("some hash"), "some-id", time.Now())
			require.NoError(t, err)
			_, err = service.ListRefresh(ctx, []byte("some hash"), 0, filterFunc, tok, []string{prj.GetPublicId()})
			require.ErrorContains(t, err, "page size must be at least 1")
		})
		t.Run("negative page size", func(t *testing.T) {
			t.Parallel()
			...
			tok, err := refreshtoken.New(ctx, time.Now(), time.Now(), resource.CredentialStore, []byte("some hash"), "some-id", time.Now())
			require.NoError(t, err)
			_, err = service.ListRefresh(ctx, []byte("some hash"), -1, filterFunc, tok, []string{prj.GetPublicId()})
			require.ErrorContains(t, err, "page size must be at least 1")
		})
		t.Run("nil filter func", func(t *testing.T) {
			t.Parallel()
			tok, err := refreshtoken.New(ctx, time.Now(), time.Now(), resource.CredentialStore, []byte("some hash"), "some-id", time.Now())
			require.NoError(t, err)
			_, err = service.ListRefresh(ctx, []byte("some hash"), 1, nil, tok, []string{prj.GetPublicId()})
			require.ErrorContains(t, err, "missing filter item callback")
		})
		t.Run("nil token", func(t *testing.T) {
			...
			_, err = service.ListRefresh(ctx, []byte("some hash"), 1, filterFunc, nil, []string{prj.GetPublicId()})
	...
	t.Run("simple pagination with aggressive filtering", func(t *testing.T) {
		filterFunc := func(ctx context.Context, l credential.Store) (bool, error) {
	...
	t.Run("simple pagination with deletion", func(t *testing.T) {
		...
		require.Equal(t, resp3.RefreshToken.GrantsHash, []byte("some hash"))
		require.False(t, resp3.CompleteListing)
		require.Equal(t, resp3.EstimatedItemCount, 4)
		...
		require.Equal(t, resp4.EstimatedItemCount, 4)
		require.Len(t, resp4.Items, 1)
		...
		require.Equal(t, resp5.RefreshToken.GrantsHash, []byte("some hash"))
		require.True(t, resp5.CompleteListing)
		require.Equal(t, resp5.EstimatedItemCount, 4)
