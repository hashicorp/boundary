	...
	if _, err := r.writer.DoTx(ctx, db.StdRetryCnt, db.ExpBackoff{}, func(rd db.Reader, w db.Writer) error {
		...
		for rows.Next() {
			var (
				...
				vaultPath, credentialType, httpMethod, httpRequestBody,
				username, keyType, ttl, keyId, criticalOptions,
				extensions, additionalValidPrincipals sql.NullString
				createTime, updateTime *timestamp.Timestamp
				version, keyBits       sql.NullInt64
				typ                    string
			)
			if err := rows.Scan(
				&publicId,
			...
			case "generic":
				...
				if httpRequestBody.String != "" {
					cl.HttpRequestBody = []byte(httpRequestBody.String)
				}
				libs = append(libs, cl)
			case "ssh":
				libs = append(libs, &SSHCertificateCredentialLibrary{
					SSHCertificateCredentialLibrary: &store.SSHCertificateCredentialLibrary{
						...
						KeyBits:                   uint32(keyBits.Int64),
						Ttl:                       ttl.String,
						KeyId:                     keyId.String,
						CriticalOptions:           criticalOptions.String,
						Extensions:                extensions.String,
						AdditionalValidPrincipals: additionalValidPrincipals.String,
					},
				})
			default:
				return errors.New(ctx, errors.Internal, op, fmt.Sprintf("unexpected vault credential library type %s returned", typ))
			}
		}
