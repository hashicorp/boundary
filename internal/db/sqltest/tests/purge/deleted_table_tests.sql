...
begin;
  ...
  as $$
    select * from collect_tap(
      has_trigger(op_table(deletion_table_name), 'trigger_insert_on_deletion'),
      trigger_is(op_table(deletion_table_name), 'trigger_insert_on_deletion', 'insert_deleted_id')
    );
  $$ language sql;
