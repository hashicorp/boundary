begin;
  ...
  select is(count(*), 1::bigint)
   ...
   where hc.scope_id  = 'p____bwidget'
  ...
  select is(count(*), 1::bigint)
    from host_set hs
    join host_catalog hc on hs.catalog_id = hc.public_id
   where hc.scope_id  = 'p____swidget'
  ...
  prepare insert_invalid_target_host_set as
    insert into target_host_set
      (target_id, host_set_id)
    values
      ('test______wb', 's___1ws-plghs');
  select throws_ok('insert_invalid_target_host_set', '23000', null, 'insert invalid target_host_set succeeded');
