...
type driver interface {
	...
	CheckHook(context.Context, migration.CheckFunc) error
	// RepairHook is a hook that runs prior to a migration's statments.
	// It should run in the same transaction a corresponding Run call.
	RepairHook(context.Context, migration.RepairFunc) (string, error)
...
func (b *Manager) runMigrations(ctx context.Context, p *provider.Provider) ([]RepairLog, error) {
	...
	for p.Next() {
		...
		if h := p.PreHook(); h != nil {
			if checkErr := b.driver.CheckHook(ctx, h.CheckFunc); checkErr != nil {
				...
				report, repairErr := b.driver.RepairHook(ctx, h.RepairFunc)
				if repairErr != nil {
					return nil, errors.Wrap(ctx, repairErr, op)
				}
