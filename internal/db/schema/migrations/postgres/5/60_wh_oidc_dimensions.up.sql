begin;
  ...
  -- TODO: Figure out if this assumes single account/user association
  create view whx_user_dimension_source as
       select -- id is the first column in the target view
              ...
              case when aa.public_id is null then 'None'
                   when aa.public_id LIKE 'acctoidc_%' then 'oidc auth account'
                   else 'password auth account'
                   end                          as auth_account_type,
              case when aa.public_id LIKE 'acctoidc_%' then coalesce(aoa.name, 'None')
                   else coalesce(apa.name, 'None')
                   end                          as auth_account_name,
              case when aa.public_id LIKE 'acctoidc_%' then coalesce(aoa.description, 'None')
              ...
              case when am.public_id is null then 'None'
                   when am.public_id LIKE 'amoidc_%' then 'oidc auth method'
                   else 'password auth method'
                   end                          as auth_method_type,
              case when am.public_id is null then 'None'
                   when am.public_id LIKE 'amoidc_%' then coalesce(aom.name, 'None')
                   else coalesce(apm.name, 'None')
                  end                          as auth_method_name,
              case when am.public_id is null then 'None'
                   when am.public_id LIKE 'amoidc_%' then coalesce(aom.description, 'None')
