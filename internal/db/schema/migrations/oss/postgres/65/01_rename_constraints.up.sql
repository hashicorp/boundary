...
begin;
  ...
  declare
    ...
    i int;
  begin
  for const in select pc.conname as constraint_name,
    pgc.relname as table_name,
  ...
  from pg_catalog.pg_constraint pc
	...
	order by pgc.relname, pc.conname
  loop
    i = (select max(x) from unnest(const.conkey) as x);
    const.new_name = const.new_name || (select pa.attname from pg_catalog.pg_attribute pa where 
      const.conrelid = pa.attrelid and pa.attnum = i) || '_fkey';
