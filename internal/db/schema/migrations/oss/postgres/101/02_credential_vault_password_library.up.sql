...
begin;
  ...
  create view credential_vault_library_issue_credentials as
    with
      ...
      password_only_override (library_id, password_attribute) as (
        select library_id,
          nullif(password_attribute, wt_to_sentinel('no override'))
    ...
    select library.public_id    as public_id,
      ...
      left join password_only_override po
  ...
  -- Replaces view created in 100/01_credential_vault_ldap_library.up.sql
