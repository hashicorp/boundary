...
begin;
  ...
  begin
    ...
    -- already ran for today
      if (date_trunc('day', now(), 'utc') - '1 day'::interval) = (select max(snapshot_date) from sessions_pending_daily_snapshot)
    then return;
    end if;

    -- never run before and there are only sessions starting from today
      if (select count(*) from sessions_pending_daily_snapshot) = 0
     and date_trunc('day', now(), 'utc') = (select min(session_pending_time) from wh_session_accumulating_fact)
    then return query
           insert into sessions_pending_daily_snapshot
             (snapshot_date, sessions_pending_count)
           values
             (date_trunc('day', now(), 'utc') - '1 day'::interval, 0)
           returning *;
         return;
    end if;
    ...
    daily_range (day) as (
        select bucket
          from generate_series(
                  coalesce(date_trunc('day', (select min(session_pending_time) from wh_session_accumulating_fact), 'utc'),
                           date_trunc('day', now(), 'utc') - '1 day'::interval),
                  now() - '1 day'::interval,
                  '1 day'::interval
               ) as bucket
    ),
