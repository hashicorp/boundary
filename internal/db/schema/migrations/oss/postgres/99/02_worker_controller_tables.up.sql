...
create table server_worker (
  ...
  worker_reported_name wt_name
    constraint worker_reported_name_or_key_id_must_be_set_by_status
      check (
          (last_status_time is null and worker_reported_name is null)
          or
          (last_status_time is not null and worker_reported_name is not null and key_id is null)
          or
          (last_status_time is not null and key_id is not null and worker_reported_name is null)
        )
...
-- key_id for a self-managed workers. If worker reported name is not provided, key_id MUST be provided.
-- This field is used purely to ensure that constraint
  key_id text
    constraint key_id_or_worker_reported_name_must_be_set_by_status
    check (
