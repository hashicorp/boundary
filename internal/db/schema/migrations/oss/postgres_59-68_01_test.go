...
func TestMigrations_AddEgressAndIngressFilters(t *testing.T) {
	...
	result, err := testRepo.LookupTarget(ctx, oldTargetId1)
	...
	result, _, err = testRepo.UpdateTarget(ctx, result, result.GetVersion(), []string{"WorkerFilter"})
	...
	result, _, err = testRepo.UpdateTarget(ctx, result, result.GetVersion(), []string{"EgressWorkerFilter"})
	...
	result, err = testRepo.LookupTarget(ctx, oldTargetId2)
	require.Equal(t, oldWorkerFilter, result.GetWorkerFilter())
	...
	result, _, err = testRepo.UpdateTarget(ctx, result, result.GetVersion(), []string{"IngressWorkerFilter"})
