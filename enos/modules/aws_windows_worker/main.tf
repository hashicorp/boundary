...
resource "aws_instance" "worker" {
  ...
  vpc_security_group_ids = var.security_group == "" ? [aws_security_group.windows_worker.id, var.security_group] : [var.security_group]
...
'@
                  ...
                  Add-Computer -DomainName "mydomain.com" -Credential $credential
...
# start the boundary server
resource "enos_local_exec" "run_boundary" {
  count = var.boundary_cli_zip_path != "" ? 1 : 0
  depends_on = [
  ...
  inline = ["ssh -i ${abspath(local_sensitive_file.private_key.filename)} -o IdentitiesOnly=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no Administrator@${aws_instance.worker.public_ip} Start-Job -ScriptBlock {boundary server -config ${local.test_dir}/worker.hcl}"]
}
